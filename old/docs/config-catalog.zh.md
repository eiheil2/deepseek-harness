 ...
 * deployment persona (forwarded to the system-prompt plugin); `toolOrder` and
 * `maxAggregateToolSchemaBytes` configure its model-facing tool schemas;
...
export interface Config {
  ...
  toolOrder?: string[]
  /** Complete model-visible tool-schema byte budget forwarded to the system-prompt plugin. */
  maxAggregateToolSchemaBytes?: NonNullable<agentCore.Config['maxAggregateToolSchemaBytes']>
...
export interface Config {
  ...
  /** Aggregate UTF-8 bytes read across one baseline discovery batch. */
 ...
 * `persona`, `toolOrder`, and `maxAggregateToolSchemaBytes` to the system-prompt
 * plugin (the fixed opener, dynamic-context policy, deployment persona, tool
 * order, and aggregate schema budget), the `tools` object to the tool registry
...
export interface Config {
  ...
  /** The complete model-visible tool-schema byte budget (see dsh-system-prompt's `Config`). */
  maxAggregateToolSchemaBytes?: SystemPromptConfig['maxAggregateToolSchemaBytes']
...
export interface Config {
  ...
  toolOrder?: string[]
  /**
   * Maximum UTF-8 JSON bytes for the complete model-visible tool-schema array.
   * Assembly fails rather than silently hiding tools when this limit is exceeded.
   */
  maxAggregateToolSchemaBytes?: number
}
```

来源：[`packages/core/system-prompt/src/index.ts:189`](../packages/core/system-prompt/src/index.ts)
