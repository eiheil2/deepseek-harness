...
Registration is a trusted same-process contract. The registry borrows the typed definition as readonly input, requires `output`, validates its raw schema, and checks semantic requirements such as a positive finite `timeoutMs` within the host timer range; `schemas()` constructs the model-facing projection when building a request, so execution and presentation share one resolved definition without leaking callbacks onto the wire.
...
Source: [`packages/core/tools/src/index.ts:794`](../../packages/core/tools/src/index.ts)
