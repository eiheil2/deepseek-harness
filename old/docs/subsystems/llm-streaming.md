...
One keep/drop decision covers content and metadata together: a `max-tokens` finish drops every tool call because a truncated call is unsafe to execute, and it drops an open block whose type cannot be reconstructed from deltas. The same decision prunes the replay envelope's per-block entry at each dropped position. `blocks()` and `replayState` therefore cannot disagree, whatever assembly removes; other finish reasons keep the strict failure for an unassemblable open block.
...
interface LlmModelContext {
  ...
  /** True when the adapter supplied a bounded fallback rather than an exact capability. */
  estimated?: boolean
}
```
