...
该基线会成为一条持久 `user/message`，并携带带类型的 `agent-instructions` 来源。其 `baseline: true` 标记将完整基线与后续增量区分开来，`baselineIdentity` 记录规范化的发现、优先级、项目根目录和预算语义，变更列表则持久保存已纳入的作用域和内容 digest。已确认缺失的根标记允许继续向上发现，而不可用的标记探测会保守地在该目录界定发现范围，使加载器无法跨入祖先项目；该区别由[根标记失败决策](../bug-fix/2026-08-18-context-root-marker-failure-boundary.md)负责。若先前排队的 workspace 基线仍在等待，插件会删除该确切消息并 prepend 替代值，而不会累积副本。
