...
The baseline becomes a durable `user/message` with a typed `agent-instructions` source. Its `baseline: true` marker distinguishes a complete baseline from later deltas, its `baselineIdentity` records normalized discovery, precedence, project-root, and budget semantics, and its change list persists the included scopes and content digests. A confirmed missing root marker permits upward discovery, while an unavailable marker probe conservatively bounds discovery at that directory so the loader cannot cross into an ancestor project; the [root-marker failure decision](../bug-fix/2026-08-18-context-root-marker-failure-boundary.md) owns that distinction. If a previously queued workspace baseline is still pending, the plugin removes that exact message and prepends its replacement instead of accumulating duplicates.
