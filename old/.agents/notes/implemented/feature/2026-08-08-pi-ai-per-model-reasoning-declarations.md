...
Current-state correction: [the distinguishable reasoning efforts Note](../bug-fix/2026-08-19-pi-ai-distinguishable-reasoning-efforts.md) partially supersedes only the rule above that every positive level carries a wire spelling. A binary OpenAI-completions dialect instead leaves its sole positive value empty and validates that declaration after compat resolution. The model-level declaration surface, compat configuration, catalog overrides, and the remaining rationale stay current.
