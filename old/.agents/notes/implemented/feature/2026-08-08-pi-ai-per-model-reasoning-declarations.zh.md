...
当前状态修正：[推理档位可区分 Note](../bug-fix/2026-08-19-pi-ai-distinguishable-reasoning-efforts.md) 仅部分取代上文「每个正向档都携带协议拼写」这条规则。OpenAI-completions 二值方言改为把唯一正向档的值留空，并在 compat 解析后校验该声明。模型级声明面、compat 配置、catalog 覆盖以及其余论证仍然有效。
