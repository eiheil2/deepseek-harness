...
These values are ordinary durable session-projection state. `@deepseek-ai/dsh-token-meter` registers four units when `ctx.sessionProjections` is present.
...
`cacheTelemetry` folds only samples that contain a cache-read or cache-write field. Its `reportedInputTokens` is the matching denominator, so an adapter that omits cache telemetry cannot dilute a reporting route's rate. The percentage is a whole-log aggregate over that reporting subset, not current-route telemetry. The pi-ai seam cannot classify its mandatory zero buckets as reported or synthesized, so it omits zeros; this avoids fabricated coverage but means the rate can be biased upward after zero-hit calls.
...
`contextBreakdown` carries heuristic system-prompt, tool-schema, and message composition for the next request. It is deliberately separate from provider-anchored `contextPressure`; the composition rows describe what the context contains and are not expected to sum to the occupancy numerator.
...
All four units ride the standard projection lifecycle: history tail baselines, `session/projection` live frames, higher-seq-wins client storage, JSON checkpoints, cache recovery, and unit unload. There is no token-specific history field, mux frame, projector, revision counter, or client fence.

The Web reads all four through the standard `useProjection` seat. `StatsLine` uses `tokenUsage` for billing and `cacheTelemetry` for the reported-subset cache rate; `ContextMeter` uses `contextPressure` and `contextBreakdown`. Window nodes still supply turn and step counts plus LLM and tool wall times — those answer "what is on screen" and are correctly window-scoped. Durable token and context groups remain when compaction leaves no visible assistant step. Cache writes count in billed input and, for reporting samples, in the cache-hit denominator. A deployment without token-meter drops the token groups; occupancy stays hidden until both pressure and capacity are known.
