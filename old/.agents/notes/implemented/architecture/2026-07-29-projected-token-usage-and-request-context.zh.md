...
这些值都是普通的持久会话投影状态。当 `ctx.sessionProjections` 存在时，`@deepseek-ai/dsh-token-meter` 会注册四个单元。
...
`cacheTelemetry` 只折算带有缓存读取或缓存写入字段的样本。其 `reportedInputTokens` 是对应的分母，所以省略缓存遥测的适配器不会稀释有报告路由的命中率。该百分比是针对完整日志中「有报告」子集的汇总，而不是当前路由的遥测。pi-ai seam 无法判定它的必填零 bucket 是明确报告还是合成值，因此会省略零值；这避免了虚构覆盖率，却也意味着经过零命中调用后，该比率可能向上偏移。
...
`contextBreakdown` 携带下一请求的系统提示词、工具 schema 与消息组成的启发式估算。它刻意与由提供方读数锚定的 `contextPressure` 分开：组成行描述上下文包含什么，不应被期待与占用率分子相加相等。
...
四个单元都沿用标准投影生命周期：历史尾页基线、`session/projection` 实时帧、seq 高者胜的客户端存储、JSON 检查点、缓存恢复和单元卸载。系统没有任何 token 专用的历史字段、mux 帧、投影器、修订计数器或客户端栅栏。

Web 通过标准 `useProjection` 席位读取全部四个单元。`StatsLine` 用 `tokenUsage` 计费，并用 `cacheTelemetry` 计算「有报告」子集的缓存命中率；`ContextMeter` 使用 `contextPressure` 和 `contextBreakdown`。窗口内节点仍提供轮次和步骤计数，以及 LLM（大语言模型）与工具的墙钟时间：它们回答的是「屏幕上有什么」，按窗口作用域正是正确的。压缩使可见 assistant 步骤归零后，持久 token 与上下文分组仍会保留。缓存写入会计入计费输入，并在样本有报告时计入缓存命中率分母。未部署 token-meter 时会去掉 token 分组；只有压力与容量都已知时才显示占用率。
...
每个会话日志会为每次路由或已公布容量变化增加一条小型 `request/context` 记录。token-meter 投影是持久会话投影用量语义的正典所有方；TUI 未挂载通用投影 seam，因此保留自己的实时逐步骤 map，而独立浏览器 fixture（测试前置数据）会镜像这些单元。ApiProxy 不携带任何 token 专用代码，不拥有逐会话指标缓存，也不执行测量。浏览器只保留四个通用投影值，不保留连接本地的遥测数据；流式文本增量仍不会迫使统计行重新计算。
