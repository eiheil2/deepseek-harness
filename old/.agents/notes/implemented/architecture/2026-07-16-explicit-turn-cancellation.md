...
The ordinary Host `session.cancel` operation preserves pending inbox work, requests the user cancellation, and awaits `Agent.whenIdle()` before acknowledging success. The RPC therefore reports convergence rather than signal admission. A signal-ignoring in-process operation delays the response, matching handle disposal and scope teardown instead of exposing a false idle state.
...
Host API coverage holds `whenIdle()` open after observing the exact `cancel({ kind: 'user' }, { keepInbox: true })` call and verifies that `session.cancel` does not acknowledge until the idle proof settles.
