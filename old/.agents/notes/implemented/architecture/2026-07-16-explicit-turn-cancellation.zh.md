...
普通 Host `session.cancel` 操作会保留待处理 inbox 工作、请求用户取消，并在确认成功前等待 `Agent.whenIdle()`。因此，该 RPC 报告的是收敛，而非 signal 已被接纳。忽略 signal 的同进程操作会延迟响应，与句柄 dispose 和作用域清理保持一致，而不会暴露虚假的空闲状态。
...
Host API 覆盖会在观察到精确的 `cancel({ kind: 'user' }, { keepInbox: true })` 调用后保持 `whenIdle()` 未结算，并验证 `session.cancel` 在空闲证明结算前不会确认成功。
