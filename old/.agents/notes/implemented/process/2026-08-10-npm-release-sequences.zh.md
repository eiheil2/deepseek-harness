...
发布只从 GitHub Actions 执行，没有本机发布路径。publish 不读 tag 来决定包含哪些包。在接触 registry 前，它会根据 checkout manifest 重建所选发布族，并要求下载的 artifact 按计算顺序包含完整成员集，使用预期的安全文件名，且 tarball 自报身份、版本和 payload 策略均匹配。缺失、额外、重复、乱序、链接、带路径穿越名称、过期或外族的 tarball 都会被拒绝。随后它才对每个已验证 tarball 拿版本与 registry 比对，分三态：
...
| `verify-packed-install` | 核验所选族的每个成员与 payload，把一个或多个 pack 目录的 tarball 装进一次性 consumer，导入所有 vendor 公开入口，并驱动已安装的 dsh 可执行入口及其 Web payload |
| `publish` | 上面那三态 |
| `process` / `tarball` | 跨平台启动命令、读取 tarball 并将其绑定到发布族的唯一正家，其中的入口守卫让每个脚本都可被 import |

dsh 族套用仓库的发布 payload 策略（拒绝源码与声明映射）。Web 前端另有包内 prepack 检查和发布族 payload 检查，要求存在 `dist/index.html` 与生成资源；packed-install 会对安装后的副本重复检查。vendored 族保留上游 payload，因为那些 manifest 导出 `./src/*`，去掉 `src` 会发出一个导出映射指向不存在文件的包。

发布子进程会直接执行原生工具。在 Windows 上，只有固定的 npm 与 pnpm 命令名通过 `cmd.exe`，因为 Node 无法直接执行它们的 `.cmd` shim；每个参数都会被引用，而命令展开字符会被拒绝，不会进入插值。
...
一个 pull request 会为两条序列跑完整的 pack（无凭据），并把每个已打包发布族装进一次性 consumer。普通 Node 会驱动 `dsh --version`，要求已安装的 Web 入口与资源存在，并通过公开根导出导入每个 vendored 包。这些探针证明发布族完整性、payload 选择、公开入口加载和发布范围解析；它们不覆盖交互式 CLI 或浏览器行为。
