...
Publication runs only from GitHub Actions; there is no local publication path. Publish reads no tag to decide what to include. Before it reaches the registry, it reconstructs the selected family from checkout manifests and requires the downloaded artifact to contain that complete member set in the computed order, under the expected safe filenames, with matching tarball identities, versions, and payload policy. It rejects missing, extra, duplicate, reordered, linked, traversal-named, stale, or foreign tarballs. For each verified tarball it then compares the version against the registry, in three states:
...
| `verify-packed-install` | verifies every selected family member and payload, installs the tarballs of one or more pack directories into a throwaway consumer, imports every vendor public entry, and drives the installed dsh executable plus its Web payload |
| `publish` | the three registry states above |
| `process` / `tarball` | the one home for cross-platform command invocation and for reading and binding packed tarballs to a family, including the entry guard that keeps every script importable |

The dsh family applies the repository's publication payload policy, which rejects sources and declaration maps. Its Web frontend also has a package-local prepack check and family payload check for `dist/index.html` and emitted assets; packed-install repeats that check against the installed copy. The vendored family keeps upstream's payload, because those manifests export `./src/*` and dropping `src` would publish an export map pointing at absent files.

Release subprocesses execute native tools directly. On Windows, only the fixed npm and pnpm command names pass through `cmd.exe`, because Node cannot execute their `.cmd` shims directly; every argument is quoted, and command-expansion characters are rejected rather than interpolated.
...
A pull request runs the full pack for both sequences without credentials and installs each packed family into a throwaway consumer. Plain Node drives `dsh --version`, the installed Web entry and assets are required, and every vendored package is imported through its public root export. These probes prove family completeness, payload selection, public entry loading, and published range resolution; they do not exercise interactive CLI or browser behavior.
