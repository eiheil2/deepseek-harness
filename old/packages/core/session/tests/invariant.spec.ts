...
describe('session-log invariants', () => {
  ...
  it('rejects a tool execution whose callId differs from the assistant block', async () => {
    const { ctx } = await setup()
    const session = ctx.sessions.create()
    session.append('turn/start', { turn: 1 })
    session.append('step/start', { turn: 1, step: 1 })
    session.append('assistant/message', {
      ...
      message: createMessage({
        role: 'assistant',
        content: [{ type: 'tool-call', id: CallId('model-call'), name: 'echo', arguments: '{}' }],
    ...
    expect(() => session.append('tool/call', {
      turn: 1,
      step: 1,
      callId: CallId('different-execution'),
      name: 'echo',
      arguments: '{}',
    })).toThrow(/not declared by an assistant tool-call block/)
  })

  it('rejects duplicate assistant tool-call identities within a step', async () => {
    const { ctx } = await setup()
    const session = ctx.sessions.create()
    session.append('turn/start', { turn: 1 })
    session.append('step/start', { turn: 1, step: 1 })
    expect(() => session.append('assistant/message', {
      ...
      message: createMessage({
        role: 'assistant',
        content: [
          { type: 'tool-call', id: CallId('duplicate'), name: 'one', arguments: '{}' },
          { type: 'tool-call', id: CallId('duplicate'), name: 'two', arguments: '{}' },
        ],
        source: { kind: 'model', provider: 'mock', model: 'mock' },
      }),
    }, { surfaceOp: 'append' })).toThrow(/repeats tool-call duplicate/)
  })
