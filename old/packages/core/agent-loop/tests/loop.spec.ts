...
describe('agent loop', () => {
  ...
  it('reprojects runtime context after a same-pre-step surface replacement', async () => {
    const adapter = new MockAdapter([textResponse('one'), textResponse('two')])
    const ctx = await harness(adapter)
    ctx.systemPrompt.context({ name: 'policy', order: 0, text: 'Mode: read-only.' })
    const agent = ctx.agentLoop.create(SessionId('a-runtime-context-same-step-compaction'), {
      provider: 'mock',
      model: 'mock',
    })
    ...
    send(agent, 'first')
    await waitForIdle(ctx, agent)
    const retained = agent.session.events.find(event =>
      event.type === 'user/message'
      && event.data.source.kind === 'plugin'
      && event.data.source.plugin === '@deepseek-ai/dsh-system-prompt')
    if (retained?.type !== 'user/message') throw new Error('first turn did not materialize runtime context')
    ...
    send(agent, 'same-step compaction')
    await waitForIdle(ctx, agent)
    const requestContexts = adapter.requests[1]?.messages.filter(message =>
      message.source.kind === 'plugin'
      && message.source.plugin === '@deepseek-ai/dsh-system-prompt') ?? []
    expect(requestContexts).toHaveLength(1)
    expect(adapter.requests[1]?.messages.at(-1)).toBe(requestContexts[0])
  })
  ...
  it('continues after max-tokens cuts off an open non-delta block', async () => {
    const adapter = new MockAdapter([[
    ...
    const ctx = await harness(adapter)
    const agent = ctx.agentLoop.create(SessionId('max-tokens-open-image'), {
      provider: 'mock',
      model: 'mock',
    })
    const reasons: TurnEndReason[] = []
  ...
  it('rejects a partial adapter stream that ends without a terminal finish', async () => {
    const adapter = new MockAdapter([[
      { type: 'text-delta', index: 0, text: 'partial response' },
    ]])
    const ctx = await harness(adapter)
    const agent = ctx.agentLoop.create(SessionId('missing-finish'), { provider: 'mock', model: 'mock' })

    send(agent, 'hi')
    await waitForIdle(ctx, agent)

    const turnEnd = agent.session.events.find(event => event.type === 'turn/end')
    expect(turnEnd?.type === 'turn/end' ? turnEnd.data.reason : undefined).toMatchObject({
      kind: 'error',
      error: {
