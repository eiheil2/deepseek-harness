...
describe('request/context capacity records', () => {
  ...
  it('records a changed estimate status when the route and capacity stay unchanged', async () => {
    let estimated = true
    const adapter = new class extends MockAdapter {
      override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({
          ...
          id: model,
          name: model,
          context: { contextWindow: 128_000, estimated },
        })
      }
    }([textResponse('a'), textResponse('b')])
    const ctx = await harness(adapter)
    const agent = ctx.agentLoop.create(SessionId('capacity-estimate-change'), { provider: 'mock', model: 'mock' })

    send(agent, 'estimated')
    await waitForIdle(ctx, agent)
    estimated = false
    send(agent, 'exact')
    await waitForIdle(ctx, agent)
    ...
    expect(agent.session.events
      ...
      { provider: 'mock', model: 'mock', contextWindow: 128_000, contextEstimated: true },
      { provider: 'mock', model: 'mock', contextWindow: 128_000, contextEstimated: false },
    ])
  })
