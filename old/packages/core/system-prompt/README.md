...
| `maxAggregateToolSchemaBytes` | `262144` | Maximum UTF-8 JSON bytes for the complete model-visible `PromptAssembly.tools` array after the assembly waterfall and complete-section/runtime-context constraints. Exceeding the limit rejects assembly before a model request; it never silently removes tools. This is a transport-size guard, not a token or context-window estimate. |
...
- `ctx.systemPrompt.assemble(context?: AssembleContext): Promise<PromptAssembly>` Assemble the prompt for one caller: the global layer merged with `context.scope`'s layer, with tool schemas detached before the transform waterfall. Runs through the scope-filtered `system-prompt/assemble` waterfall, then restores an effective complete section as the sole prompt section, enforces any active runtime-context suppressor, and checks the complete tool-schema byte budget. An optional `context.signal` explicitly controls this assembly request; providers and listeners may cooperate with it but must not retain it for another turn. Rejects for multiple complete sections, when a configured `toolOrder` names a tool outside the providers' `knownNames` universe, when a provider returns the reserved rest-entry name, or when final tool schemas exceed `maxAggregateToolSchemaBytes`.
...
Schema tokens repeat on every request. Restricting a tool removes its entire schema cost for that agent but not a separate prompt section; reordering changes cache shape but not semantic content. The aggregate byte budget bounds serialized JSON size, not provider tokenization.
