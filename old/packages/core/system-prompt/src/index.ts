...
/** Default UTF-8 byte budget for the complete model-visible tool-schema array. */
export const DEFAULT_MAX_AGGREGATE_TOOL_SCHEMA_BYTES = 256 * 1024
...
export class SystemPrompt extends Service {
  static Config: z<Config> = z.object({
    ...
    maxAggregateToolSchemaBytes: z.number()
      .step(1)
      .min(1)
      .max(Number.MAX_SAFE_INTEGER)
      .default(DEFAULT_MAX_AGGREGATE_TOOL_SCHEMA_BYTES),
  })
  ...
  private readonly maxAggregateToolSchemaBytes: number

  constructor(ctx: Context, config: Config) {
    super(ctx, 'systemPrompt')
    this.toolOrder = validateToolOrder(config.toolOrder)
    this.maxAggregateToolSchemaBytes = config.maxAggregateToolSchemaBytes
      ?? DEFAULT_MAX_AGGREGATE_TOOL_SCHEMA_BYTES
  ...
  async assemble(context: AssembleContext = {}): Promise<PromptAssembly> {
    ...
    const finalAssembly = completeSection === undefined && !runtimeContextSuppressed
      ? transformed
      : {
        ...transformed,
    ...
    const toolSchemaBytes = new TextEncoder().encode(JSON.stringify(finalAssembly.tools)).byteLength
    if (toolSchemaBytes > this.maxAggregateToolSchemaBytes) {
      throw new Error(
        `aggregate tool schemas require ${toolSchemaBytes} UTF-8 bytes, exceeding maxAggregateToolSchemaBytes=${this.maxAggregateToolSchemaBytes}`,
      )
    }
