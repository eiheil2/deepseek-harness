...
- `ctx.tools.register(definition: ToolDefinition): () => void`：注册一个受信任、带类型的同进程定义，其中必须包含规范的 `output` 声明。所在层由调用上下文的作用域决定：普通插件上下文会全局注册；agent 的 `agent.ctx` 只为该 agent 注册，并在此处遮蔽同名全局工具。同一层内名称重复会抛出；非原生模式还会拒绝保留的 `run_code` 传输名称。缺失或不受支持的输出声明，以及不在主机定时器正有限范围内的 `timeoutMs`，都会使注册失败。可选的同步 `finalizeContent` 回调会在调用开始时纳入快照；在所有流水线结果（包括实体化其他结果字段时发现的错误）规范化之后，它只能替换最终面向模型的内容。该注册会随调用方 fiber 一同 dispose（资源释放）。
...
- `ctx.tools.execute(exec)`：以无损方式快照并冻结参数、分配不透明 token，并在运行完整的策略／分发／结果流水线之前捕获当时精确可见的注册代次。若卸载、限制、遮蔽或同名重新注册在分发前改变了该代次，已接纳的调用会以 `UNKNOWN_TOOL` 失败；只有后续调用才会路由到新注册。无效参数会进入同一结果路径，但不会到达策略或工具主体。环绕包装层只能替换 `signal`；注册表会在进入工具主体之前，立即将调用方的原始信号重新合并到当前信号中。
...
可选的 `timeoutMs` 必须为正数、有限值，且不得超过 `2,147,483,647` 毫秒这一主机定时器最大延迟。注册表会在工具变为可见之前拒绝无法执行的声明；该字段仍是策略元数据，而非模型可见的 schema。
...
每次 `run_code` 执行都会取得新的加密执行命名空间，并使用 `<root>:code:<execution-namespace>:<n>` 作为嵌套分发 id。命名空间在第一次 await 之前分配，因此提供方调用 id 即使跨轮次、注册表重载或进程重启复用，持久化的子事件仍可单独寻址，且不需要保留无界的内存历史。
