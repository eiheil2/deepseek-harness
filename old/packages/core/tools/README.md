...
- `ctx.tools.execute(exec)` losslessly snapshots and freezes arguments, assigns an opaque token, and captures the exact visible registration generation before running the complete policy/dispatch/result pipeline. If unload, restriction, shadowing, or same-name re-registration changes that generation before dispatch, the admitted call fails as `UNKNOWN_TOOL`; only later calls route to the new registration. Invalid arguments use the same result path without reaching policy or the body. Around wrappers may replace only `signal`; the registry re-fuses the original caller signal immediately before the body.
...
Optional `timeoutMs` must be positive, finite, and no greater than `2,147,483,647` milliseconds, the largest host timer delay. The registry rejects an unenforceable declaration before the tool becomes visible; the field remains policy metadata rather than model-visible schema.
...
Nested dispatch ids use a fresh cryptographic execution namespace for every `run_code` execution: `<root>:code:<execution-namespace>:<n>`. The namespace is allocated before the first await, so repeated provider call ids remain individually addressable across turns, registry reloads, and process restarts without retaining an unbounded in-memory history.
