...
describe('ToolRuntime', () => {
  ...
  it('does not execute a raw tool registered after an unknown call entered pre-policy', async () => {
    const ctx = await setup()
    ...
    const pending = ctx.tools.execute({
      ...
      callId: CallId('late-registration'),
      name: 'late',
      arguments: {},
    })
    ...
    ctx.tools.register({
      name: 'late',
      description: 'registered after the call started',
  ...
  it('does not dispatch a replacement definition under the original concurrency classification', async () => {
    const ctx = await setup()
    const entered = Promise.withResolvers<undefined>()
    const release = Promise.withResolvers<undefined>()
    const calls: string[] = []
    ...
    expect(ctx.tools.executionMode(input)).toEqual({ kind: 'parallel' })
    ctx.on('tools/pre-execute', async (_exec, next) => {
      entered.resolve(undefined)
      await release.promise
      return next()
    ...
    const pending = ctx.tools.execute(input)
    await entered.promise
    dispose()
    ctx.tools.register({
      ...echoTool,
  ...
  it('treats dispose and re-register of the same definition object as a new registration', async () => {
    const ctx = await setup()
    const entered = Promise.withResolvers<undefined>()
    const release = Promise.withResolvers<undefined>()
    const calls: string[] = []
    ...
    const pending = ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('same-object-reregister'),
      name: 'same-object',
      arguments: {},
    })
    await entered.promise
  ...
  it('rejects a registration timeout outside the host timer range', async () => {
    const ctx = await setup()
    expect(() => ctx.tools.register({ ...echoTool, name: 'zero-timeout', timeoutMs: 0 }))
      .toThrow(`timeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`)
...
describe('defineTool validation (the runtime-validation Agent Note, part 1)', () => {
  ...
  it('throws when timeoutMs is outside the host timer range', () => {
    const make = (ms: number) => defineContentToolFixture({
      name: 'x', description: 'd', parameters: {}, timeoutMs: ms,
      async execute() { return [{ type: 'text' as const, text: 'ok' }] },
    })
    expect(() => make(0)).toThrow(`timeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`)
    expect(() => make(-5)).toThrow('positive finite number')
    expect(make(MAX_TIMER_DELAY_MS).timeoutMs).toBe(MAX_TIMER_DELAY_MS)
    expect(() => make(MAX_TIMER_DELAY_MS + 1)).toThrow(`no greater than ${MAX_TIMER_DELAY_MS}`)
  })
