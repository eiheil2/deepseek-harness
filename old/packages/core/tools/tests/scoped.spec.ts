...
describe('scoped tool registration', () => {
  ...
  it('does not let a late scoped shadow replace the definition of an admitted call', async () => {
    const ctx = await mount()
    const { scope, key } = await mintAgentScope(ctx, 'a')
    ...
    const pending = run(ctx, 'shared', key)
    ...
    await expect(pending).resolves.toBe('Error: unknown tool "shared"')
