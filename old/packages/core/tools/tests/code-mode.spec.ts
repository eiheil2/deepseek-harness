...
async function runCode(
  ctx: Context,
  code: string,
  extras: { agent?: Agent; signal?: AbortSignal; description?: string; callId?: string } = {},
): Promise<ToolExecutionResult> {
  return ctx.tools.execute({
    signal: testToolSignal,
    callId: CallId(extras.callId ?? 'call-1'),
    name: RUN_CODE_NAME,
...
describe('the sub-dispatch scheduler (native concurrency contract)', () => {
  ...
  it('overlaps concurrency-safe calls under Promise.all and logs a start event per dispatch', async () => {
    ...
    expect(starts.map(start => start.subCallId)).toEqual(expect.arrayContaining([
      expect.stringMatching(/^call-1:code:[0-9a-f-]{36}:1$/),
      expect.stringMatching(/^call-1:code:[0-9a-f-]{36}:2$/),
      expect.stringMatching(/^call-1:code:[0-9a-f-]{36}:3$/),
    ]))
    expect(new Set(starts.map(start => start.subCallId))).toHaveLength(3)
    expect(new Set(settles.map(settle => settle.subCallId))).toEqual(new Set(starts.map(start => start.subCallId)))
  })

  it('keeps nested dispatch ids unique when a provider reuses a root call id', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    registerEcho(ctx)
    const { agent, events } = fakeAgent()
    runtime.behavior = async request => ({
      logs: [],
      value: await request.bindings[0]!.functions.echo!({ value: 'ok' }),
    })

    const first = await runCode(ctx, 'await tools.echo({ value: "ok" })', { agent, callId: 'duplicate-root' })
    const second = await runCode(ctx, 'await tools.echo({ value: "ok" })', { agent, callId: 'duplicate-root' })
    ...
    expect(starts[0]).toMatch(/^duplicate-root:code:[0-9a-f-]{36}:1$/)
    expect(starts[1]).toMatch(/^duplicate-root:code:[0-9a-f-]{36}:1$/)
    expect(starts[0]).not.toBe(starts[1])
    expect(new Set(starts)).toHaveLength(2)
  ...
  it('ordered pre-execute never overlaps: a slow policy on one call delays the next start', async () => {
    ...
    runtime.behavior = async (request) => {
      ...
      expect(stages[0]).toMatch(/^pre-enter:call-1:code:[0-9a-f-]{36}:1$/)
      releaseGate!()
      await expect.poll(() => gated.pending()).toBe(2)
      gated.releaseAll()
      await all
    ...
    expect(stages[0]).toMatch(/^pre-enter:call-1:code:[0-9a-f-]{36}:1$/)
    expect(stages[1]).toMatch(/^pre-exit:call-1:code:[0-9a-f-]{36}:1$/)
    expect(stages[2]).toMatch(/^pre-enter:call-1:code:[0-9a-f-]{36}:2$/)
    expect(stages[3]).toMatch(/^pre-exit:call-1:code:[0-9a-f-]{36}:2$/)
  })
  ...
  it('post-execute and context commitment stay in submission order under out-of-order completion', async () => {
    ...
    expect(postOrder).toHaveLength(2)
    expect(postOrder[0]).toMatch(/^call-1:code:[0-9a-f-]{36}:1$/)
    expect(postOrder[1]).toMatch(/^call-1:code:[0-9a-f-]{36}:2$/)
    // Deferred contexts reach the outer result in the same order.
    const contextTexts = result.additionalContexts?.map(c => (c.content[0] as { text: string }).text)
    expect(contextTexts).toHaveLength(2)
    expect(contextTexts?.[0]).toMatch(/^ctx:call-1:code:[0-9a-f-]{36}:1$/)
    expect(contextTexts?.[1]).toMatch(/^ctx:call-1:code:[0-9a-f-]{36}:2$/)
  })
  ...
  it('a queued-unstarted call abandoned by run settlement logs no start event', async () => {
    ...
    expect(starts[0]).toMatch(/^call-1:code:[0-9a-f-]{36}:1$/)
    expect(settles).toEqual(starts)
...
describe('the run_code dispatch bridge', () => {
  it('bridges tool calls, returns only the curated output, and logs one event per dispatch', async () => {
    ...
    expect(dispatches[0]).toMatchObject({
      rootCallId: 'call-1', parentCallId: 'call-1', name: 'echo',
      arguments: { value: 'one' }, isError: false, content: [{ type: 'text', text: 'echo:one' }],
    })
    expect(dispatches[0]?.subCallId).toMatch(/^call-1:code:[0-9a-f-]{36}:1$/)
    expect(dispatches[1]).toMatchObject({
      rootCallId: 'call-1', parentCallId: 'call-1', name: 'echo',
      arguments: { value: 'two' }, isError: false, content: [{ type: 'text', text: 'echo:two' }],
    })
    expect(dispatches[1]?.subCallId).toMatch(/^call-1:code:[0-9a-f-]{36}:2$/)
  ...
  it('defers sub-call additionalContexts onto the outer run_code result', async () => {
    ...
    const result = await runCode(ctx, 'program')
    expect(result.isError).toBe(false)
    const contextTexts = result.additionalContexts?.map(context =>
      (context.content[0] as { text: string }).text)
    expect(contextTexts).toHaveLength(2)
    expect(contextTexts?.[0]).toMatch(/^context for call-1:code:[0-9a-f-]{36}:1$/)
    expect(contextTexts?.[1]).toMatch(/^context for call-1:code:[0-9a-f-]{36}:2$/)
  })
