...
export interface ToolDefinition extends ToolSchema {
  ...
  /**
   * Cooperative tool-call timeout budget in milliseconds, no greater than
   * the host timer maximum. Omit for no deadline.
...
export interface CodeDispatchLog {
  ...
  readonly agent?: Agent
  /** Unique per-execution sub-call id (`<parent>:code:<execution-namespace>:<n>`). */
...
interface ToolView {
  /** Visible registration generations after restrictions, scoped shadowing, and transport insertion. */
...
/** One effect-owned registration generation, distinct even when a definition object is reused. */
...
export class ToolRuntime extends Service {
  ...
  /** Visible registration generation captured before policy; registry churn cannot reroute an admitted call. */
  private executionRegistrations = new WeakMap<ToolExecution, ToolRegistration | undefined>()
  ...
  /** Stable registration identity for the reserved transport's process lifetime. */
  private codeTransportRegistration: ToolRegistration | undefined
  ...
  /** Stable registration generation for the reserved transport. */
  private requireCodeTransportRegistration(): ToolRegistration {
    this.codeTransportRegistration ??= { definition: this.requireCodeTransport() }
    return this.codeTransportRegistration
  }
  ...
  private wireSchemas(scope?: ScopeKey): ToolProviderResult {
    ...
    if (mode === 'native') {
      const schemas = [...view.visible.values()].map(registration => this.schemaOf(registration.definition, false))
  ...
  register(definition: ToolDefinition): () => void {
    ...
    if (timeoutMs !== undefined
      ...
      throw new TypeError(
        `tool "${name}" timeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`,
      )
    }
  ...
  private view(scope?: ScopeKey): ToolView {
    ...
    for (const layer of layers) {
      ...
      for (const [name, registration] of layer.tools.entries()) inherited.set(name, registration)
    }
    ...
    for (const [name, registration] of inherited) {
      ...
      if (layers.every(layer => layer.admits(name))) visible.set(name, registration)
    }
    ...
    if (this.modeFor(scope) !== 'native') {
      visible.set(RUN_CODE_NAME, this.requireCodeTransportRegistration())
    }
  ...
  /** Resolve the exact registration generation that may execute for this call. */
  private resolveRegistration(name: string, scope: ScopeKey | undefined, nested: boolean): ToolRegistration | undefined {
    const registration = this.view(scope).visible.get(name)
  ...
  private resolveExecution(name: string, scope: ScopeKey | undefined, nested: boolean): ToolDefinition | undefined {
    return this.resolveRegistration(name, scope, nested)?.definition
  }
  ...
  schemas(scope?: ScopeKey): ToolSchema[] {
    return [...this.view(scope).visible.values()].map(registration => this.schemaOf(registration.definition, true))
  }
  ...
  private createExecution(exec: ToolExecutionInput): ScheduledToolPreparation | { kind: 'ready'; exec: MutableToolRunContext } {
    ...
    const visibleRegistration = this.view(agent).visible.get(name)
    const visible = visibleRegistration?.definition
    ...
    try {
      ...
      this.executionRegistrations.set(execution, collapsed ? undefined : visibleRegistration)
  ...
  /**
   * Resolve only the exact registration admitted when this call entered the
   * runtime. Removing, shadowing, restricting, or replacing it invalidates
   * this not-yet-dispatched call instead of rerouting it to another body.
   */
  private activeExecutionDefinition(exec: ToolExecution): ToolDefinition | undefined {
    const captured = this.executionRegistrations.get(exec)
    ...
    return current === captured ? captured.definition : undefined
  }
  ...
  private async dispatchToolBody(exec: MutableToolRunContext): Promise<ToolExecutionResult> {
    ...
    exec.signal = signal
    try {
      const tool = this.activeExecutionDefinition(exec)
