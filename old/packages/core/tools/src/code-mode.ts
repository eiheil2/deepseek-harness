...
export function createRunCodeTool(registry: ToolRuntime, options: RunCodeBridgeOptions): ToolDefinition {
  ...
  const definition = defineTool({
    ...
    async execute(args, exec): Promise<RunCodeOutput> {
      ...
      const rootCallId = String(exec.callId)
      // Provider call ids may repeat across turns, sessions, registry reloads,
      // and processes. Generate the child namespace before the first await so
      // concurrent runs never share it and no registry-lifetime Set can grow.
      ...
      const binding = (name: string): CodeBindingFunction => async (rawArgs: unknown): Promise<JsonValue> => {
        ...
        const n = ++dispatches
        const subCallId = CallId(`${rootCallId}:code:${runNamespace}:${n}`)
        const input = {
          callId: subCallId,
