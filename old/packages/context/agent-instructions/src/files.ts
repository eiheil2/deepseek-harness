...
async function probeMarker(path: string, fileSystem?: FileSystem, signal?: AbortSignal): Promise<MarkerProbe> {
 ...
 * @returns the discovered project root, the first directory with an unavailable
 * marker probe, or `cwd` when no marker exists.
 */
export async function findProjectRoot(
  ...
  for (;;) {
    ...
    for (const marker of markers) {
      const probe = await probeMarker(join(current, marker), fileSystem, signal)
      if (probe === 'present') return current
      if (probe === 'unavailable') unavailable = true
    }
...
export async function loadBaselineInstructionSet(
  ...
  for (const file of discovered) {
    // Stat-known files can be rejected before opening them; unknown-size
    // providers are bounded after the read and never admitted past the cap.
    if (file.size !== undefined && aggregateBytes + file.size > config.maxAggregateSourceBytes) continue
