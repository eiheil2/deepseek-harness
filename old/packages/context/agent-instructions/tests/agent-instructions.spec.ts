...
class RecordingFileSystem extends FileSystem {
  ...
  override async stat(target: FsTarget, signal?: AbortSignal): Promise<FsInfo | undefined> {
    ...
    const statError = this.statErrors.get(target.targetKey)
...
describe('workspace context instruction discovery', () => {
  ...
  it('bounds host discovery at a non-missing marker probe error', async () => {
    const root = await tempRepo()
    try {
      const cwd = join(root, 'nested')
      await mkdir(cwd, { recursive: true })

      await expect(findProjectRoot(cwd, ['\0'])).resolves.toBe(resolve(cwd))
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('keeps cwd as the project root when its marker lookup is unavailable', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('does not cross an unavailable cwd marker into ancestor project instructions', async () => {
    const outer = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      fs.entries.set(join(outer, 'AGENTS.md'), { type: 'file', content: 'outer project rule' })
      fs.entries.set(join(cwd, 'AGENTS.md'), { type: 'file', content: 'nested project rule' })
      await ctx.plugin(workspaceContext, { dshHome: home, maxBytes: 65536 })
      const agent = stubAgent(cwd)

      await composeBaselinePrefix(ctx, agent)

      expect(derivedText(agent)).toContain('nested project rule')
      expect(derivedText(agent)).not.toContain('outer project rule')
    } finally {
      await rm(outer, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
  ...
  it('continues past a provider-confirmed missing marker to a visible ancestor project', async () => {
    const outer = await tempRepo()
    const home = await tempRepo()
    try {
      const cwd = join(outer, 'nested')
      ...
      await ctx.plugin(RecordingFileSystem)
      const fs = ctx.fs as RecordingFileSystem
      fs.statErrors.set(join(cwd, '.git'), Object.assign(new Error('missing marker'), { code: 'ENOENT' }))
      fs.entries.set(join(outer, '.git'), { type: 'directory' })
      fs.entries.set(join(outer, 'AGENTS.md'), { type: 'file', content: 'visible outer rule' })
      fs.entries.set(join(cwd, 'AGENTS.md'), { type: 'file', content: 'visible nested rule' })
      await ctx.plugin(workspaceContext, { dshHome: home, maxBytes: 65536 })
      const agent = stubAgent(cwd)

      await composeBaselinePrefix(ctx, agent)

      expect(derivedText(agent)).toContain('visible outer rule')
      expect(derivedText(agent)).toContain('visible nested rule')
    } finally {
      await rm(outer, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
