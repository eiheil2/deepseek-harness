...
function visibleContextTexts(session: Session): string[] {
  const texts: string[] = []
  for (const message of session.deriveMessages()) {
    if (message.role === 'user'
      && message.source.kind === 'plugin'
      && message.source.plugin === 'tmux-context') {
      texts.push(message.content.find(block => block.type === 'text')?.text ?? '')
    }
  }
  return texts
}
...
describe('tmux-context injection', () => {
  ...
  it.each(['live', 'restored'] as const)(
    're-injects unchanged state after compaction in a %s session',
    async (continuation) => {
      vi.useFakeTimers()
      vi.setSystemTime(1_000)
      const { ctx } = await mount({ refreshIntervalMs: 1_000 }, true)
      const original = Session.create(SessionId(`compacted-${continuation}`))
      ...
      const reading = original.events.find(event => event.type === 'user/message'
        && event.data.source.kind === 'plugin'
        && event.data.source.plugin === 'tmux-context')
      ...
      expect(visibleContextTexts(original)).toHaveLength(0)

      const session = continuation === 'live'
        ? original
        : Session.create(SessionId('compacted-restored-target'), [...original.events])
      ...
      expect(contextTexts(session)).toHaveLength(2)
      expect(visibleContextTexts(session)).toHaveLength(1)
      expect(visibleContextTexts(session)[0]).toContain('tmux location (turn 2):')
    },
  )
})
