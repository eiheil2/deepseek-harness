...
State is pulled on every eligible turn — a moved, renamed, or re-laid-out pane is picked up without any tmux hook or background process. The plugin suppresses an unchanged state only while its latest reading remains model-visible. Compaction that shadows the reading re-arms injection after the configured interval, restoring one current snapshot without discarding the durable schedule.

## Timing semantics

The plugin prepends an `agent/pre-step` listener. When an injection is due and the downstream decision enters the proposed step, it prepends one sourced `UserMessage` to the returned batch. AgentLoop records that context after `step/start` with source `{ kind: 'plugin', plugin: 'tmux-context' }`. Interval scheduling scans raw durable events for the latest injection time, while unchanged-state suppression also requires that event to remain on the current session surface. Both rules survive compaction and resumed processes without process-local cache state; sessions schedule independently. A downstream pre-step listener that rejects or fails prevents the reading from being recorded.
