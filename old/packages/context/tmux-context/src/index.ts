...
/**
 * The stable state block of this plugin's latest durable injection, its event
 * time, and whether that event remains on the current model surface. Scans raw
 * durable events so interval scheduling survives compaction and resumed
 * processes without process-local cache state.
 */
function latestInjectedState(agent: Agent): { state: string; time: number; visible: boolean } | undefined {
  ...
  for (const event of [...agent.session.events].reverse()) {
    if (event.type === 'user/message'
      ...
      return { state, time: event.time, visible: visibleSeqs.has(event.seq) }
    }
  }
  return undefined
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.on('agent/pre-step', async (
    ...
    if (previous?.visible === true && previous.state === state) return decision
