...
describe('dsh-acp-demo composition', () => {
  ...
  it('forwards the aggregate tool-schema budget through agent-spine-demo', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      maxAggregateToolSchemaBytes: 1,
      persistenceRoot: '/tmp/dsh-acp-demo-test-tool-budget',
      workspaceContext: false,
    })
