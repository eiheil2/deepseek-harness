...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.maxAggregateToolSchemaBytes !== undefined
      ? { maxAggregateToolSchemaBytes: config.maxAggregateToolSchemaBytes }
      : {},
 ...
 * forwarded system-prompt config. Workspace-context receives its own explicitly
 * forwarded config. Load order is irrelevant (cordis
...
export function apply(ctx: Context, config: Config): void {
  ...
  // Owner schemas resolve defaults; forward optional ordering and budgets only when explicitly set.
