...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('picks shared spine config without leaking entry-point fields', () => {
    const appConfig = {
      ...
      toolOrder: ['zulu'],
      maxAggregateToolSchemaBytes: 4096,
    ...
    expect(agentCore.pickSpineConfig(appConfig)).toEqual({
      ...
      maxAggregateToolSchemaBytes: appConfig.maxAggregateToolSchemaBytes,
  ...
  it('forwards maxAggregateToolSchemaBytes to the system-prompt assembly', async () => {
    const ctx = await mount({ maxAggregateToolSchemaBytes: 1, workspaceContext: false })
    ctx.get('tools')!.register({
      name: 'alpha',
      description: 'alpha',
      parameters: {},
    ...
    await expect(ctx.get('systemPrompt')!.assemble())
      .rejects.toThrow('exceeding maxAggregateToolSchemaBytes=1')
    await ctx.fiber.dispose()
  })
