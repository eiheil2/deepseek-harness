...
- `register(ns, schema, { base?, applies? })` — 返回 owner 的 `SettingsScope`（`get`/`watch`/`update`）。注册是调用方插件 fiber 上的 effect：dispose（资源释放）该 fiber 会移除 namespace、阻止已排队但未启动的观察者回调，并等待已在执行的回调结算；保留下来的已 dispose scope 会拒绝注册新 watcher（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-registration-quiescence.md)）。schema 拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
...
- `update(ns, patch)` — 把普通对象 patch 深合并进用户分节（绝不合并进 `base`），校验解析候选值，经提供方持久化后提交。patch 只能包含与 JSON 兼容的数据：Date、Map、BigInt、非有限数或循环引用会在任何内容持久化前被拒绝，并给出以 `$` 为根的路径（YAML/JSON 存储在重载时会静默改变这类值）。所有合法 JSON 属性名（包括 `__proto__` 和 `constructor`）在克隆、合并、比较和 wire 脱敏过程中始终保持为 own data property（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-json-property-safety.md)）。校验失败在持久化前拒绝；只读提供方（`writable: false`）拒绝一切写入。同一 namespace 的写入按调用顺序串行。
...
- 服务卸载先拒绝新写入与观察者调用的启动，再排干全部排队写入与已启动的观察者调用后才完成。registrant fiber 在写入途中被 dispose 时，该写入仍到达存储；若不存在替代注册则不通知任何人，若已有替代注册，则用后者自己的 schema 重新解析已持久化分节，并接收一次来源为 `provider` 的更新。不兼容的分节会保留替代注册的最后可用值（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-replacement-resync.md)）。
