...
describe('redactSecrets', () => {
  ...
  it('preserves JSON property names as own data while rebuilding object and dict values', () => {
    const input = JSON.parse('{"__proto__":{"marker":"own-data"},"constructor":{"marker":"constructor-data"}}') as Record<string, unknown>
    for (const schema of [
      { type: 'object', dict: {} },
      { type: 'dict' },
    ]) {
      const { value } = redactSecrets(schema as never, input)
      expect(Object.getPrototypeOf(value)).toBe(Object.prototype)
      expect(Object.hasOwn(value as object, '__proto__')).toBe(true)
      expect((value as Record<string, unknown>)['__proto__']).toEqual({ marker: 'own-data' })
