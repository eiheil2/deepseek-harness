...
describe('registration', () => {
  ...
  it('describes inherited values without inventing a user layer and contains malformed storage', async () => {
    const { ctx, provider } = await boot()
    ctx.settings.register(settingsNamespace('ui-theme'), ThemeSchema, {
      base: { fontSize: 16 },
    })

    const inherited = ctx.settings.describe({ redactSecrets: true })[0]!
    expect(inherited.base).toEqual({ fontSize: 16 })
    expect(inherited.user).toBeUndefined()

    provider.pushExternal({ 'ui-theme': 'malformed' })
    const contained = ctx.settings.describe({ redactSecrets: true })[0]!
    expect(contained.value).toEqual({ theme: 'dark', fontSize: 16 })
    expect(contained.base).toEqual({ fontSize: 16 })
    expect(contained.user).toBeUndefined()
  })
...
describe('update', () => {
  ...
  it('preserves JSON property names without changing the merged object prototype', async () => {
    const { ctx, provider } = await boot()
    const scope = ctx.settings.register(settingsNamespace('json-keys'), z.any())
    const patch = JSON.parse('{"__proto__":{"marker":"own-data"},"constructor":{"marker":"constructor-data"}}') as Record<string, unknown>

    await scope.update(patch)

    const stored = provider.persisted[0]!.section
    expect(Object.getPrototypeOf(stored)).toBe(Object.prototype)
    expect(Object.hasOwn(stored, '__proto__')).toBe(true)
    expect(stored['__proto__']).toEqual({ marker: 'own-data' })
    expect(Object.hasOwn(stored, 'constructor')).toBe(true)
    expect(stored['constructor']).toEqual({ marker: 'constructor-data' })
    expect(Object.hasOwn(scope.get() as object, '__proto__')).toBe(true)
  })
...
describe('deepEqualJson', () => {
  ...
  it('does not treat an inherited __proto__ accessor as an own JSON key', () => {
    const ownProto = JSON.parse('{"__proto__":{}}') as Record<string, unknown>
    expect(deepEqualJson(ownProto, { other: 1 })).toBe(false)
  })
})
...
describe('second review regressions', () => {
  ...
  it('resyncs a replacement registration after the old owner persists in flight', async () => {
    const { ctx, provider } = await boot({ persistDelayMs: 30 })
    const events = recordUpdates(ctx)
    const ns = settingsNamespace('ui-theme')
    ...
    await oldOwner
    const pending = oldScope!.update({ theme: 'light' })
    await new Promise(resolve => setTimeout(resolve, 5))
    ...
    const newOwner = ctx.plugin({
      inject: ['settings'],
      apply: (child: Context) => {
        replacement = child.settings.register(ns, ThemeSchema)
  ...
  it('keeps a replacement last-good when an old owner persists an incompatible value', async () => {
    const { ctx, provider } = await boot({ persistDelayMs: 30 })
    const events = recordUpdates(ctx)
    const ns = settingsNamespace('ui-theme')
    let oldScope: SettingsScope<ThemeConfig> | undefined
    ...
    const DarkOnlySchema = z.object({ theme: z.union(['dark'] as const).default('dark') })
    let replacement: SettingsScope<{ theme: 'dark' }> | undefined
    const newOwner = ctx.plugin({
      inject: ['settings'],
      apply: (child: Context) => {
        replacement = child.settings.register(ns, DarkOnlySchema)
      },
    })
    await newOwner
    await pending
    ...
    expect(events).toEqual([])
    expect(ctx.settings.describe()[0]!.revision).toBe(0)
    await newOwner.dispose()
  })
...
describe('third review regressions', () => {
  ...
  it('waits for an in-flight watch invocation at service dispose', async () => {
    ...
    expect(() => scope.watch(() => {})).toThrow(/service was disposed/)
  })
...
describe('watch', () => {
  it('keeps registrant disposal pending until an in-flight watcher settles', async () => {
    const { ctx, provider } = await boot()
    ...
    const fiber = ctx.plugin({
      ...
      apply: (child: Context) => {
        ...
        scope.watch(async () => {
          enterWatcher()
          await watcherGate
          if (disposeFinished) callbacksAfterDispose += 1
        })
      },
    })
    await fiber
    ...
    const disposing = fiber.dispose().then(() => { disposeFinished = true })
    await new Promise(resolve => setTimeout(resolve, 20))
    const finishedBeforeRelease = disposeFinished
    releaseWatcher()
    await disposing
    expect(finishedBeforeRelease).toBe(false)
    expect(callbacksAfterDispose).toBe(0)
  })

  it('rejects watcher registration through a disposed owner scope', async () => {
    const { ctx } = await boot()
    let scope: SettingsScope<ThemeConfig> | undefined
    const fiber = ctx.plugin({
      inject: ['settings'],
    ...
    await fiber
    await fiber.dispose()
    expect(() => scope!.watch(() => {})).toThrow(/registration was disposed/)
  })
...
describe('installSettingsSection', () => {
  ...
  it('drives the source through attach, live commits, and detach', async () => {
    ...
    let changes = 0
    const validate = vi.fn<(value: { theme: string }) => void>()
    ...
    expect(validate).toHaveBeenCalledWith({ theme: 'user' })
