...
- `register(ns, schema, { base?, applies? })` — returns the owner `SettingsScope` (`get`/`watch`/`update`). The registration is an effect on the calling plugin's fiber: disposing that fiber removes the namespace, prevents queued observer callbacks from starting, and waits for callbacks already in flight. A retained disposed scope rejects new watcher registration ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-registration-quiescence.md)). A stored section the schema rejects fails the registration itself; a duplicate namespace fails loud.
...
- `update(ns, patch)` — deep-merges the plain-object patch into the user section only (never the `base`), validates the resolved candidate, persists through the provider, then commits. Patches may contain only JSON-compatible data: a Date, Map, BigInt, non-finite number, or circular reference rejects with its `$`-rooted path before anything persists (YAML/JSON storage would silently change such values on reload). Every valid JSON property name, including `__proto__` and `constructor`, remains an own data property through cloning, merge, comparison, and wire redaction ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-json-property-safety.md)). Validation failure rejects before anything is persisted; a read-only provider (`writable: false`) rejects every write. Writes to one namespace are serialized in call order.
...
- Service teardown refuses new writes and watcher starts, then drains every queued write and every started watcher invocation before disposal completes. A write whose registrant fiber was disposed mid-flight still reaches storage; with no replacement registration it notifies nobody, while a replacement re-resolves the persisted section under its own schema and receives a `provider` update. An incompatible section keeps the replacement's last good value ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-18-settings-replacement-resync.md)).
