...
/** Define one enumerable own data property without invoking `__proto__` accessors. */
  ...
  const clone = (value: unknown, path: string): unknown => {
    ...
    if (isPlainObject(value)) {
      ...
      for (const [key, entry] of Object.entries(value)) {
        ...
        defineJsonProperty(out, key, clone(entry, `${path}.${key}`))
      }
      visiting.delete(value)
      return out
    }
...
function mergeLayers(under: unknown, over: unknown): unknown {
  ...
  for (const [key, value] of Object.entries(over)) {
    ...
    defineJsonProperty(merged, key, next)
  }
  return merged
}
...
export abstract class SettingsProvider extends Service {
  ...
  register<T>(ns: SettingsNamespace, schema: z<T>, options?: SettingsRegisterOptions<T>): SettingsScope<T> {
    ...
    return {
      ...
      watch: (callback) => {
        if (this.isStopped()) {
          throw new Error(`settings service was disposed before watcher registration for "${ns}"`)
        }
        if (this.registrations.get(ns) !== registration) {
          throw new Error(`settings namespace "${ns}" registration was disposed before watcher registration`)
        }
  ...
  private write(
    ...
    const run = previous.catch(() => undefined).then(async () => {
      ...
      const beforePersistedSection = this.document[ns]
      this.document[ns] = section
      if (this.registrations.get(ns) === registration && !this.isStopped()) {
        this.bumpRevision(registration, beforePersistedSection, section)
        this.commit(registration, next, 'update')
        return
      }
      const replacement = this.registrations.get(ns)
      if (replacement === undefined || this.isStopped()) return
      let replacementNext: unknown
      try {
        replacementNext = deepFreeze(this.resolve(
          replacement.schema,
          replacement.base,
          section,
        ...
        this.ctx.logger.warn('settings: keeping last good replacement "%s" after an old owner persisted', ns)
        this.ctx.logger.warn(error)
        return
      }
      this.bumpRevision(replacement, beforePersistedSection, section)
      this.commit(replacement, replacementNext, 'provider')
    })
