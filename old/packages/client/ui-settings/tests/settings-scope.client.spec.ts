...
describe('SettingsScopeController', () => {
  ...
  it('keeps the prior revision when a superseded read settles before a failed newer read', async () => {
    ...
    await scope.load()
    const superseded = scope.load()
    const latest = scope.load()
    stale.resolve(described({ preference: 'light' }, 4))
