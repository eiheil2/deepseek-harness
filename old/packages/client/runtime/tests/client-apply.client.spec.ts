...
describe('runtime client apply', () => {
  ...
  it('forwards reconnect disconnects so the replacement workspace pull owns public state', async () => {
    const bench = await mount()
    ...
    let calls = 0
    bench.api.onList = () => Promise.resolve(ok({ items: [] }))
    bench.api.onWorkspaceList = () => calls++ === 0 ? oldPull.promise : newPull.promise

    bench.sinks?.onConnected?.({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true })
    expect(bench.api.callsOf('workspace.list')).toHaveLength(1)
    bench.sinks?.onStateChange?.('reconnecting')
    bench.sinks?.onConnected?.({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true })
    expect(bench.api.callsOf('workspace.list')).toHaveLength(2)

    oldPull.resolve(ok({ items: [], archivedSessionIds: ['old-archive'] as never[] }))
    await flushMicrotasks()
    newPull.resolve(ok({ items: [], archivedSessionIds: ['new-archive'] as never[] }))
    await flushMicrotasks()
    const workspaces = bench.ctx.get('workspaces') as WorkspaceRuntime
    expect(workspaces.list.getSnapshot().archivedSessionIds).toEqual(['new-archive'])
  })
