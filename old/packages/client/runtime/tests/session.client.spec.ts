...
describe('paging', () => {
  ...
  it('drops an older-page response superseded by resync without settling the fresh page load', async () => {
    const { api, session } = makeSession()
    api.onHistory = () => histResponse(plainTurn(12, 2, 'initial', 'tail'), true)
    await session.open()
    ...
    api.onHistory = () => histResponse(plainTurn(18, 3, 'fresh', 'tail'), true)
    ...
    stalePage.resolve(ok({
      events: entries(plainTurn(6, 1, 'stale', 'page')) as never[],
      hasMore: false,
      modelSelection: { provider: 'deepseek-official', model: 'stale' },
    }))
    await staleLoad
    expect(session.getSnapshot()).toMatchObject({ hasMore: true, loadingOlder: true })
    expect(session.getSnapshot().nodes.map(n => n.seq)).toEqual([19, 21])

    freshPage.resolve(ok({
      events: entries(plainTurn(12, 2, 'fresh', 'page')) as never[],
      hasMore: false,
      modelSelection: { provider: 'deepseek-official', model: 'fresh' },
    }))
    await freshLoad
    ...
    expect(session.getSnapshot().nodes.map(n => n.seq)).toEqual([13, 15, 19, 21])
  })
})
