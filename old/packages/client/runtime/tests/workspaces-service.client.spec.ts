...
describe('WorkspaceManager', () => {
  ...
  it('does not let a pre-reconnect list pull overwrite the new generation', async () => {
    const api = new FakeApiClient()
    const oldPull = deferred<Awaited<ReturnType<FakeApiClient['onWorkspaceList']>>>()
    const newPull = deferred<Awaited<ReturnType<FakeApiClient['onWorkspaceList']>>>()
    let calls = 0
    api.onWorkspaceList = () => calls++ === 0 ? oldPull.promise : newPull.promise
    const manager = new WorkspaceManager(api)

    const oldOperation = manager.refresh()
    manager.handleDisconnected()
    manager.handleConnected()
    const newOperation = manager.refresh()
    expect(calls).toBe(2)
    expect(newOperation).not.toBe(oldOperation)

    oldPull.resolve(ok({ items: [workspace('old')] as never[] }))
    await oldOperation
    expect(manager.refresh()).toBe(newOperation)
    manager.handleHostEnvelope({
      rpcId: 'after-old-settlement' as never,
      payload: { type: 'host/workspace-changed', workspace: workspace('frame') },
    })

    newPull.resolve(ok({ items: [workspace('new')] as never[] }))
    await newOperation
    expect(manager.getSnapshot().items.map(item => item.workspaceId)).toEqual(['frame', 'new'])
  })
