...
describe('list lifecycle', () => {
  ...
  it('does not let a pre-reconnect list pull overwrite the new generation', async () => {
    const api = new FakeApiClient()
    const oldPull = deferred<Awaited<ReturnType<FakeApiClient['onList']>>>()
    const newPull = deferred<Awaited<ReturnType<FakeApiClient['onList']>>>()
    let calls = 0
    api.onList = () => calls++ === 0 ? oldPull.promise : newPull.promise
    const manager = new SessionManager(api, fakeRemote())

    const oldOperation = manager.refreshList()
    manager.handleDisconnected()
    manager.handleConnected()
    const newOperation = manager.refreshList()
    expect(calls).toBe(2)
    expect(newOperation).not.toBe(oldOperation)

    // The old request settles while the replacement is still in flight. Its
    // finally block must not clear the replacement's single-flight state.
    oldPull.resolve(ok({ items: [summary(S1)] as never[] }))
    await oldOperation
    expect(manager.refreshList()).toBe(newOperation)
    manager.handleHostEnvelope({
      rpcId: 'after-old-settlement' as never,
      payload: { type: 'host/session-added', sessionId: S2, blank: false },
    })

    newPull.resolve(ok({ items: [summary(S1)] as never[] }))
    await newOperation
...
describe('subagent catalogs', () => {
  it('does not let a pre-reconnect catalog pull overwrite the new generation', async () => {
    const api = new FakeApiClient()
    const root = 'fk-root' as SessionId
    const oldPull = deferred<Awaited<ReturnType<FakeApiClient['onSubagentList']>>>()
    const newPull = deferred<Awaited<ReturnType<FakeApiClient['onSubagentList']>>>()
    let calls = 0
    api.onSubagentList = () => calls++ === 0 ? oldPull.promise : newPull.promise
    const manager = new SessionManager(api, fakeRemote(), root)

    const oldOperation = manager.refreshSubagents(root)
    manager.handleDisconnected()
    manager.handleConnected()
    const newOperation = manager.refreshSubagents(root)
    expect(newOperation).not.toBe(oldOperation)
    await Promise.resolve()
    expect(calls).toBe(2)

    // Settling the old pull must neither install its stale catalog nor delete
    // the replacement generation's single-flight record.
    oldPull.resolve(ok({
      entries: [{
        kind: 'child', id: S1, mode: 'continuable', label: 'old',
        activity: 'inactive', hasChildren: false,
      }] as never[],
      parentAvailable: false,
    }))
    await oldOperation
    expect(manager.refreshSubagents(root)).toBe(newOperation)

    newPull.resolve(ok({
      entries: [{
        kind: 'child', id: S2, mode: 'continuable', label: 'new',
        activity: 'inactive', hasChildren: false,
      }] as never[],
  ...
  it('cancels a pending catalog refresh when the connection generation ends', async () => {
    vi.useFakeTimers()
    try {
      const api = new FakeApiClient()
      const manager = new SessionManager(api, fakeRemote())
      ...
      const baseline = api.callsOf('subagent.list').length
      manager.handleHostEnvelope({
        rpcId: 'child-added-before-disconnect' as never,
        payload: {
          type: 'host/session-added', sessionId: S2, parentSessionId: S1, blank: false,
        },
      })
      ...
      expect(api.callsOf('subagent.list')).toHaveLength(baseline)
    } finally {
      vi.useRealTimers()
    }
  })
