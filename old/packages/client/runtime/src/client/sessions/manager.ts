...
export class SessionManager {
  ...
  /** Monotone connection generation for list pulls; stale unary responses cannot install state. */
  ...
  /** Monotone connection generation for catalog pulls; stale responses cannot install state. */
  ...
  refreshSubagents(parentSessionId: SessionId): Promise<void> {
    ...
    const operation = (async () => {
      try {
        ...
        if (generation !== this.catalogGeneration || inflight?.token !== token) return
        if (result.ok) {
          ...
          this.catalogs.set(parentSessionId, {
            ...
            parentAvailable: inflight.parentAvailableOverride
  ...
  refreshList(): Promise<void> {
    ...
    const operation = (async () => {
      ...
      // A disconnected request may settle after a newer generation has
      // started. It must not clear the newer mutation buffer or in-flight
      // promise, nor publish a notification for stale state.
      if (this.listInflight === operation) {
        this.listMutations = null
        this.listInflight = null
        this.notifier.markDirty()
      }
    })
    this.listInflight = operation
    return operation
  }
