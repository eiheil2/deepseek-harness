...
断连会使两类在途列表拉取失效。重连会启动新的拉取，死亡连接代次的迟到响应或结算既不能安装状态，也不能清除替代请求。
...
每个 `Session` 都把连续事件窗口交给 `ConversationNodeAssembler`。插件注册业务 Definition，把单个事件映射为稳定的 `{kind, id}`，在唯一 start 事件处创建 State，折叠有关联的 update，再为已注册的视图目标构造最终节点。Assembler 负责 Context 索引、只读前序 Context 查询，以及引用稳定的 Turn/Step Location 索引。实时 append 只对每个 Definition 求值一次，并且只更新命中的 Context；加载更早分页时保留已有 Context 与节点身份，只匹配新 prepend 的事件，并重放前序依赖或 Location 事实发生变化的 Context。重连 resync 会使上一连接代次的旧页请求失效、立即释放其加载状态，并阻止该请求的响应或结算改变重建后的窗口。完整替换仅用于 open、resync 和 gap repair。
