...
export class ConversationController extends Service implements IConversation {
  ...
  private readonly sendChains = new Map<SessionId, Promise<void>>()
  ...
  async send(text: string): Promise<void> {
    ...
    await this.enqueueSend(session, async () => {
  ...
  async sendSession(
    ...
    await this.enqueueSend(session, async (assertCurrent) => {
  ...
  /** Serialize prompt admissions per session while retaining cross-session concurrency. */
  private enqueueSend(
    session: SessionFace,
    operation: (assertCurrent: () => void) => Promise<void>,
  ): Promise<void> {
    if (this.disposed) return Promise.reject(new Error('conversation send target was disposed'))
    const sessions = this.requireSessions()
    const assertCurrent = (): void => {
      if (this.disposed) throw new Error('conversation send target was disposed')
      if (sessions.binding(session.sessionId)?.session !== session) {
        throw new Error(`conversation send target session "${session.sessionId}" was released`)
      }
    }
    ...
    const previous = this.sendChains.get(session.sessionId)
    const started = previous === undefined
      ? Promise.resolve().then(run)
      : previous.then(run, run)
    const tracked = started.finally(() => {
      if (this.sendChains.get(session.sessionId) === tracked) this.sendChains.delete(session.sessionId)
    })
    this.sendChains.set(session.sessionId, tracked)
    return tracked
  }
