...
describe('ConversationController', () => {
  ...
  it('preserves per-session submit order while an earlier image is being read', async () => {
    const b = await bench()
    const bytes = Promise.withResolvers<ArrayBuffer>()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:ordered-image')
    try {
      const file = new File([Uint8Array.of(1)], 'slow.png', { type: 'image/png' })
      ...
      const first = b.root.sendSession(session, 'first', [attachment.id], 'queue')
      await vi.waitFor(() => { expect(read).toHaveBeenCalledOnce() })
      const second = b.scoped.send('second')
      ...
      expect(b.prompt).toHaveBeenNthCalledWith(1, [
        { type: 'image', mediaType: 'image/png', data: 'AQ==', name: 'slow.png' },
        { type: 'text', text: 'first' },
      ], 'queue')
      expect(b.prompt).toHaveBeenNthCalledWith(2, [{ type: 'text', text: 'second' }], 'queue')
    } finally {
      created.mockRestore()
      await b.runtime.dispose()
    }
  ...
  it('continues a session send chain after an earlier business failure', async () => {
    const b = await bench()
    b.prompt.mockResolvedValueOnce({
      ok: false, error: { code: 'agent-busy', message: 'busy', details: {} },
    } as never)

    const first = b.scoped.send('first')
    const second = b.scoped.send('second')

    await expect(first).rejects.toThrow('conversation.send failed: agent-busy: busy')
    await expect(second).resolves.toBeUndefined()
    expect(b.prompt).toHaveBeenNthCalledWith(1, [{ type: 'text', text: 'first' }], 'queue')
    expect(b.prompt).toHaveBeenNthCalledWith(2, [{ type: 'text', text: 'second' }], 'queue')
    await b.runtime.dispose()
  })

  it('does not serialize prompt admission across sessions', async () => {
    const b = await bench()
    const blocked = Promise.withResolvers<Awaited<ReturnType<SessionFace['prompt']>>>()
    b.prompt.mockReturnValueOnce(blocked.promise as never)
    const first = b.scoped.send('blocked')
    await vi.waitFor(() => { expect(b.prompt).toHaveBeenCalledOnce() })

    const prompt2 = vi.fn(() => Promise.resolve({ ok: true as const, value: { accepted: true as const } }))
    ...
    const scoped2 = b.runtime.sessions.scope('s2')?.get('conversation') as ConversationController | undefined
    if (scoped2 === undefined) throw new Error('second session scope missing')

    await scoped2.send('free')
    expect(prompt2).toHaveBeenCalledWith([{ type: 'text', text: 'free' }], 'queue')

    blocked.resolve({ ok: true, value: { accepted: true } })
    await first
    await b.runtime.dispose()
  })

  it('does not prompt after disposal finishes during image serialization', async () => {
    const b = await bench()
    const bytes = Promise.withResolvers<ArrayBuffer>()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:disposed-image')
    try {
      const file = new File([Uint8Array.of(1)], 'slow.png', { type: 'image/png' })
      const read = vi.fn(() => bytes.promise)
      Object.defineProperty(file, 'arrayBuffer', { value: read })
      ...
      const session = b.runtime.sessions.binding('s1')?.session
      if (session === undefined) throw new Error('session missing')
      const pending = b.root.sendSession(session, '', [attachment.id], 'queue')
      await vi.waitFor(() => { expect(read).toHaveBeenCalledOnce() })

      await b.fiber.dispose()
      bytes.resolve(Uint8Array.of(1).buffer)

      await expect(pending).rejects.toThrow('conversation send target was disposed')
      expect(b.prompt).not.toHaveBeenCalled()
    } finally {
      created.mockRestore()
      await b.runtime.dispose()
  ...
  it('does not prompt a released session after image serialization', async () => {
    const b = await bench()
    const bytes = Promise.withResolvers<ArrayBuffer>()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:released-image')
    try {
      const file = new File([Uint8Array.of(1)], 'slow.png', { type: 'image/png' })
      const read = vi.fn(() => bytes.promise)
      Object.defineProperty(file, 'arrayBuffer', { value: read })
      ...
      b.shell.commitSend([attachment.id])
      const session = b.runtime.sessions.binding('s1')?.session
      if (session === undefined) throw new Error('session missing')
      const pending = b.root.sendSession(session, '', [attachment.id], 'queue')
      await vi.waitFor(() => { expect(read).toHaveBeenCalledOnce() })
      ...
      const replacementPrompt = vi.fn(() => Promise.resolve({
      ...
      await expect(pending).rejects.toThrow('conversation send target session "s1" was released')
      expect(b.prompt).not.toHaveBeenCalled()
