...
describe('ContextMeter', () => {
  ...
  it('shows the occupancy ring and opens the breakdown panel on click', () => {
    ...
    expect(panel.textContent).not.toContain('~32K / 128K')
  ...
  it('draws no bar segment at zero occupancy', () => {
    ...
    expect(panel.textContent).not.toContain('~0 / 128K')
  })
  ...
  it('marks the capacity reading when the model context window is estimated', () => {
    const view = meter({
      contextPressure: {
    ...
    fireEvent.click(view.getByRole('button', { name: '上下文已用 25%' }))
    expect(view.container.querySelector('[role="dialog"]')!.textContent).toContain('~32K / 128K')
  })
