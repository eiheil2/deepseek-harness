...
export class ConnectionController {
  ...
  /** Read through a method because abort state can change across awaits. */
  private isLifecycleActive(lifecycle: AbortSignal): boolean {
    return !lifecycle.aborted
  }

  /** Re-read both mutable liveness guards after a potentially reentrant sink. */
  private isGenerationActive(controller: AbortController, lifecycle: AbortSignal): boolean {
    return this.isLifecycleActive(lifecycle) && !controller.signal.aborted
  }

  private async loop(lifecycle: AbortSignal): Promise<void> {
    while (this.isLifecycleActive(lifecycle)) {
      ...
      try {
        ...
        if (this.isGenerationActive(ac, lifecycle)) {
      ...
      await failed
      if (!this.isLifecycleActive(lifecycle)) return
