...
function buildAlphaLog(): SessionEvent[] {
  ...
  // keyed registration a top-level bash row uses).
  {
    const turn = 65
    const callId = `fx-call-${turn}`
    const executionNamespace = `fixture-exec-${turn}`
    ...
    const dispatchPair = (n: number, name: string, dispatchArgs: Record<string, unknown>, resultText: string, isError = false): void => {
      push({
        ...
        data: {
          rootCallId: callId, parentCallId: callId,
          subCallId: `${callId}:code:${executionNamespace}:${n}`,
          name, arguments: dispatchArgs,
        },
      })
      push({
        ...
        data: {
          rootCallId: callId, parentCallId: callId,
          subCallId: `${callId}:code:${executionNamespace}:${n}`, name,
