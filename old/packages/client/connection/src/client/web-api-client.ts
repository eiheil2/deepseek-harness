...
export class WebApiClient extends AbstractApiClient {
  ...
  private async *readWebSocket<F extends MuxFrame | HostFrame>(
    ...
    const enqueue = (item: SocketItem<F>): void => {
      ...
      if (inbox.length >= MAX_SOCKET_INBOX) {
        // Continuing after dropping frames would corrupt ordering. End this
        // generation so the controller can reconnect and refetch a baseline.
        overflowed = true
        inbox.length = 0
        inbox.push({ kind: 'end' })
        if (socket.readyState === WebSocket.CONNECTING || socket.readyState === WebSocket.OPEN) {
          socket.close(1013, 'client backlog overflow')
        }
        wake?.()
        wake = undefined
        return
