...
function responseDisconnected(res: ServerResponse, signal: AbortSignal): boolean {
  return signal.aborted || res.destroyed
}

async function waitForDrainOrDisconnect(res: ServerResponse, signal: AbortSignal): Promise<boolean> {
  return await new Promise<boolean>((resolve) => {
    ...
    // close can fire synchronously inside write(), before these listeners exist.
    if (responseDisconnected(res, signal)) settle(false)
  })
}
...
export async function bridge(
  ...
  for await (const chunk of response.body) {
    if (responseDisconnected(res, abort.signal)) break
    ...
    // resolves so a mid-wait disconnect can't park this loop forever. The helper
    // rechecks level state after subscribing because close can race with write().
    if (!res.write(chunk) && !await waitForDrainOrDisconnect(res, abort.signal)) break
  }
  if (!responseDisconnected(res, abort.signal) && !res.writableEnded) res.end()
}
