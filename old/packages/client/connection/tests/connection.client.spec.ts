...
describe('connection lifecycle', () => {
  ...
  it('does not revive a stopped reconnect loop after an immediate restart', async () => {
    const api = new FakeApiClient()
    let connected = 0
    let restarted = false
    ...
    const controller = new ConnectionController(api, {
      onConnected: () => { connected++ },
      onStateChange: (state) => {
        if (state === 'reconnecting' && !restarted) {
          restarted = true
          controller.stop()
          controller.start()
        }
      },
    }, { ...FAST, backoffBaseMs: 40, backoffMaxMs: 40 })
    controller.start()
    try {
      await vi.waitFor(() => { expect(connected).toBe(1) })
      api.failStreams(new Error('restart during backoff'))
      await vi.waitFor(() => { expect(connected).toBe(2) })
      await new Promise(resolve => setTimeout(resolve, 40))
