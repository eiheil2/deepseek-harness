...
describe('connection client apply', () => {
  ...
  it('closes an overflowing browser inbox and ends that stream generation', async () => {
    ;(globalThis as Win).location = {
      hostname: 'localhost', search: '', origin: 'http://localhost:3080',
    }
    ;(globalThis as WebSocketGlobal).WebSocket = FakeWebSocket as unknown as typeof WebSocket
    ...
    for (let index = 0; index <= 4_096; index++) {
      socket.receive(JSON.stringify({
        type: 'server-request',
        rpcId: `overflow-${index}`,
        method: 'session/subscribed',
        payload: { type: 'session/subscribed', sessionId: 'overflow-session', lastSeq: index },
      }))
    }
    ...
    expect(socket.closeReason).toBe('client backlog overflow')
  })
