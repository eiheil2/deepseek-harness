...
describe('createFixtureApi', () => {
  ...
  it('emits namespaced Code Mode child ids with stable start and settle pairing', async () => {
    const api = createFixtureApi()
    const history = await api.sessions.history(req({
      sessionId: sid('fx-alpha'),
      maxMessages: 200,
    }))
    if (!history.result.ok) throw new Error('history failed')

    const codeEvents = history.result.value.events
      .map(entry => entry.event)
      .filter(event =>
        event.type === 'tool/code-dispatch-start' || event.type === 'tool/code-dispatch')
    const ids = codeEvents.map(event => event.data.subCallId)

    expect(ids).toEqual([
      'fx-call-65:code:fixture-exec-65:1',
      'fx-call-65:code:fixture-exec-65:1',
      'fx-call-65:code:fixture-exec-65:2',
      'fx-call-65:code:fixture-exec-65:2',
      'fx-call-65:code:fixture-exec-65:3',
      'fx-call-65:code:fixture-exec-65:3',
    ])
  })
