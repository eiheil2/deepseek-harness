...
describe('retract', () => {
  ...
  it('reclaims settled queues for packages that were never loaded', async () => {
    const bench = await boot()
    for (let index = 0; index < 1000; index++) {
      bench.runner.retract(`never-${index}` as CordisDynamicPluginId, RUN)
    }
    await bench.settle()
    const queues = (bench.runner as unknown as { queues: Map<unknown, unknown> }).queues
...
describe('observation and disposal', () => {
  ...
  it('waits for an in-flight load and prevents it from reviving after disposal', async () => {
    const bench = await boot()
    const gate = Promise.withResolvers<undefined>()
    ;(globalThis as { __runnerLoadGate?: Promise<void> }).__runnerLoadGate = gate.promise
    ;(globalThis as { __runnerLoadStarted?: boolean }).__runnerLoadStarted = false
    const loading = bench.runner.load(half({
      code: `
        globalThis.__runnerLoadStarted = true
        await globalThis.__runnerLoadGate
        return { apply() {} }
      `,
    }))
    await vi.waitFor(() => {
      expect((globalThis as { __runnerLoadStarted?: boolean }).__runnerLoadStarted).toBe(true)
    })

    let disposed = false
    const disposal = bench.runner.dispose().then(() => { disposed = true })
    await Promise.resolve()
    expect(disposed).toBe(false)
    expect(bench.runner.isLoaded(PLUGIN)).toBe(false)

    gate.resolve(undefined)
    await expect(loading).rejects.toThrow('runner is disposed')
    await disposal
    expect(disposed).toBe(true)
    expect(bench.runner.isLoaded(PLUGIN)).toBe(false)
    expect(bench.removed).toEqual([])

    delete (globalThis as { __runnerLoadGate?: Promise<void> }).__runnerLoadGate
    delete (globalThis as { __runnerLoadStarted?: boolean }).__runnerLoadStarted
  })

  it('rejects loads submitted after disposal begins', async () => {
    const bench = await boot()
    await bench.runner.dispose()
    await expect(bench.runner.load(half())).rejects.toThrow('runner is disposed')
  })
