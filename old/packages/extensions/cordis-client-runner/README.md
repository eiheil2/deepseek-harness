...
Disposal closes admission before it waits for operations already admitted to those queues. A load that finishes evaluation or loader activation after disposal began is removed instead of entering live state; a later load rejects, and settled queue keys are reclaimed. Disposal does not cancel arbitrary package code and therefore waits for admitted work to settle.
