...
export class DynamicCordisPackageRunner {
  ...
  /** Set once disposal starts; no new package may pass the mount gate after this. */
  ...
  dispose(): Promise<void> {
    ...
    this.disposed = true
    this.disposal = this.disposeNow()
    return this.disposal
  }

  private async disposeNow(): Promise<void> {
    this.unwatch()
    // A load may be evaluating or waiting for its loader fiber. Let every
    // operation already admitted to a per-package queue reach a terminal state
    // before tearing down the remaining live set.
  ...
  private enqueue<T>(id: CordisDynamicPluginId, op: () => Promise<T>): Promise<T> {
    ...
    // Keep the map bounded for package ids that are no longer active. The
    // identity guard is required when a new operation is enqueued before an
    // older tail's reaction runs.
    void tail.then(() => {
  ...
  private assertActiveWith(styles: DynamicCordisStyles): void {
    if (!this.disposed) return
    styles.dispose()
    throw disposedError()
  }
...
function disposedError(): Error {
  return new Error('cordis-client-runner: runner is disposed')
}
