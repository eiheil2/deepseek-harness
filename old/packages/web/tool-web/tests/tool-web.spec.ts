...
describe('tool-call timeout budget is plugin config', () => {
  ...
  it.each([
    ['fetchTimeoutMs', { fetch: false, fetchTimeoutMs: MAX_TIMER_DELAY_MS + 1 }],
    ['searchTimeoutMs', { search: false, searchTimeoutMs: MAX_TIMER_DELAY_MS + 1 }],
  ] as const)('rejects an over-range %s at load', async (key, config) => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(WebRuntime, {})
    await expect(ctx.plugin(ToolWeb, config))
      .rejects.toThrow(`tool-web: ${key} must be no greater than ${MAX_TIMER_DELAY_MS}`)
  })
})
