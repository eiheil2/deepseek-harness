...
export function apply(ctx: Context, config: Config): void {
  ...
  if (resolved.fetchTimeoutMs > MAX_TIMER_DELAY_MS) {
    throw new Error(`tool-web: fetchTimeoutMs must be no greater than ${MAX_TIMER_DELAY_MS}`)
  }
  if (resolved.searchTimeoutMs > MAX_TIMER_DELAY_MS) {
    throw new Error(`tool-web: searchTimeoutMs must be no greater than ${MAX_TIMER_DELAY_MS}`)
  }
