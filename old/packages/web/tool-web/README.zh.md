...
| `fetchTimeoutMs` | `30000` | `web_fetch` 的正数协作式工具调用超时预算（ms），不得大于 [`MAX_TIMER_DELAY_MS`](../../util/timeout/README.md)。 |
| `searchTimeoutMs` | `30000` | `web_search` 的正数协作式工具调用超时预算（ms），不得大于 [`MAX_TIMER_DELAY_MS`](../../util/timeout/README.md)。 |
