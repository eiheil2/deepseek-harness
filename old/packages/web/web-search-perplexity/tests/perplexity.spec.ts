...
describe('PerplexitySearchProvider error handling', () => {
  ...
  it.each(['request', 'success-body', 'error-body'] as const)(
    'maps a custom abort reason during %s to WEB_ABORTED',
    async (stage) => {
      const controller = new AbortController()
      const entered = Promise.withResolvers<undefined>()
      const reason = new Error('custom abort reason')
      const abortable = (signal: AbortSignal): Promise<Response> => new Promise((_resolve, reject) => {
        entered.resolve(undefined)
        signal.addEventListener('abort', () => { reject(reason) }, { once: true })
      })
      vi.stubGlobal('fetch', vi.fn((_url: string, init?: RequestInit) => {
        const signal = init?.signal as AbortSignal
        if (stage === 'request') return abortable(signal)
        return Promise.resolve({
          ok: stage === 'success-body',
          status: stage === 'success-body' ? 200 : 500,
          json: () => abortable(signal),
        } as unknown as Response)
      }))

      const search = new PerplexitySearchProvider(options).search({ query: 'q' }, controller.signal)
