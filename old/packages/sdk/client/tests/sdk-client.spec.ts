...
describe('HarnessClient', () => {
  it.each([
    ...
    ['disposeEofGraceMs', Number.POSITIVE_INFINITY],
    ['disposeGraceMs', 2_147_483_648],
  ] as const)('rejects an unenforceable %s before starting the runtime', (name, value) => {
    expect(() => new HarnessClient(fakeLaunch({}, { [name]: value })))
      .toThrow(`${name} must be a positive finite number no greater than 2147483647`)
  })

  it('rejects an unenforceable per-call timeout before starting the runtime', async () => {
    const client = new HarnessClient(fakeLaunch())
    cleanups.push(() => client.close())
    await expect(client.request('initialize', {}, 2_147_483_648))
      .rejects.toThrow('timeoutMs must be a positive finite number no greater than 2147483647')
