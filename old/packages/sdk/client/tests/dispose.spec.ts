...
describe('disposeRuntimeProcess', () => {
  it.each([
    [{ disposeEofGraceMs: 0, disposeGraceMs: 1000 }, 'disposeEofGraceMs'],
    [{ disposeEofGraceMs: 1000, disposeGraceMs: 2_147_483_648 }, 'disposeGraceMs'],
  ] as const)('rejects unenforceable grace periods before touching the child', async (graces, name) => {
    const fake = new FakeChild()
    await expect(disposeRuntimeProcess(asChild(fake), graces))
      .rejects.toThrow(`${name} must be a positive finite number no greater than 2147483647`)
