...
import { assertSdkTimerDelay } from './timer.ts'
...
export class HarnessClient {
  ...
  constructor(readonly options: HarnessClientOptions) {
    for (const name of ['requestTimeoutMs', 'shutdownTimeoutMs', 'disposeEofGraceMs', 'disposeGraceMs'] as const) {
      const value = options[name]
      if (value !== undefined) assertSdkTimerDelay(value, name)
    }
  }
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    if (timeoutMs !== undefined) assertSdkTimerDelay(timeoutMs, 'timeoutMs')
    this.start()
