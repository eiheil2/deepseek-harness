...
export interface HarnessClientOptions {
  ...
  /** Per-request timeout (ms), positive and at most Node's timer maximum; `undefined` waits indefinitely. */
  requestTimeoutMs?: number
  /** Positive bound (ms) on the protocol `shutdown` exchange inside `close()` (default 1000). */
  shutdownTimeoutMs?: number
  /** Positive grace (ms) for the runtime's stdin-EOF quiesce during `close()` (default 6000). */
  disposeEofGraceMs?: number
  /** Positive termination window (ms) after SIGTERM/SIGKILL during `close()` (default 3000). */
