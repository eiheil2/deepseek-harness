...
Every configured timeout and per-call `request(..., timeoutMs)` override must be a positive finite number no greater than Node's `2147483647` ms timer maximum. Invalid launch options fail in the constructor, and an invalid per-call override fails before the runtime starts; values are never silently clamped to a shorter delay.
