...
describe('editing a composition file', () => {
  ...
  it('reclaims a superseded generation after its last joined agent is disposed', async () => {
    const { scoped, path } = await editable('reclaimed-agent')
    const first = await scoped.agents.create({
      sessionId: SessionId('sess-reclaimed-first'),
      setup: async (agentCtx: Context) => void await scoped.agentPresets.mount(agentCtx, 'reclaimed-agent'),
    })
    await writeFile(path, rowFor('afterwards'))
    const second = await scoped.agents.create({
      sessionId: SessionId('sess-reclaimed-second'),
      setup: async (agentCtx: Context) => void await scoped.agentPresets.mount(agentCtx, 'reclaimed-agent'),
    })

    expect(livePresetMounts().filter(mount => mount.presetId === 'reclaimed-agent')).toHaveLength(2)
    await first.dispose()
    expect(livePresetMounts().filter(mount => mount.presetId === 'reclaimed-agent')).toHaveLength(1)
    expect(toolNames(scoped, second.agent)).toEqual(['afterwards'])
  })

  it('holds a superseded generation for a bounded reader and reclaims it on release', async () => {
    const { scoped, path } = await editable('leased-reader')
    const first = await scoped.agentPresets.acquireStandingKey('leased-reader')
    await writeFile(path, rowFor('afterwards'))
    const second = await scoped.agentPresets.acquireStandingKey('leased-reader')

    expect(livePresetMounts().filter(mount => mount.presetId === 'leased-reader')).toHaveLength(2)
    await first.release()
    expect(livePresetMounts().filter(mount => mount.presetId === 'leased-reader')).toHaveLength(1)
    await first.release()
    await second.release()
    expect(livePresetMounts().filter(mount => mount.presetId === 'leased-reader')).toHaveLength(1)
  })

  it('keeps the legacy bare standing key pinned across a refresh', async () => {
    const { scoped, path } = await editable('legacy-reader')
    await scoped.agentPresets.standingKeyFor('legacy-reader')
    await writeFile(path, rowFor('afterwards'))
    const current = await scoped.agentPresets.acquireStandingKey('legacy-reader')
    await current.release()

    // No release capability accompanied the old bare key, so reclaiming its
    // generation would invalidate a caller that still retains it.
    expect(livePresetMounts().filter(mount => mount.presetId === 'legacy-reader')).toHaveLength(2)
  })
