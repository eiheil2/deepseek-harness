...
- `ctx.agentPresets.acquireStandingKey(id?): Promise<StandingKeyLease>` Acquires the standing scope key for a bounded host read. Release the lease in `finally`; this lets superseded generations be reclaimed after their last agent and reader leave. Refuses a broken preset like `mount()`.
- `ctx.agentPresets.standingKeyFor(id?): Promise<ScopeKey>` Compatibility API for callers that retain a bare key. It pins that generation until whole-tree teardown because no release boundary is observable; new bounded reads should use `acquireStandingKey()`.
