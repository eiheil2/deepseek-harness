...
export class AgentPresets extends Service {
   ...
   * the generation they run on. A superseded generation is reclaimed after
   * its last joined agent and bounded host reader release it; the legacy
   * bare-key API pins one because it has no observable release boundary.
   */
  private readonly standing = new Map<string, Promise<StandingMount>>()

  /** Internal generation state by its opaque key, for synchronous child joins. */
  private readonly standingByKey = new WeakMap<ScopeKey, StandingMount>()
   ...
   */
  private readonly bindings = new WeakMap<ScopeKey, AgentStandingBinding>()
  ...
  async mount(agentCtx: Context, id?: string): Promise<AgentPreset> {
    ...
    const standing = await this.retainCurrentStanding(preset, current => current.joinedAgents++)
    ...
    this.bindAgent(agentCtx, agentKey, standing, true)
    return preset
  }
  ...
  composeFrom(agentCtx: Context, parentCtx: Context): string | undefined {
    ...
    const generation = this.standingByKey.get(standing.key)
    if (generation === undefined) {
      throw new Error('agent-presets: parent joined a standing generation not owned by this roster')
    }
    this.bindAgent(agentCtx, agentKey, generation)
   ...
   * so the old composition stays for its other agents and the new one is
   * ensured BEFORE the link moves. An unknown or unusable preset
  ...
  async recompose(agentCtx: Context, id: string): Promise<AgentPreset> {
    ...
    if (joined === undefined) {
      ...
      if (joined.standing === standing) {
        standing.joinedAgents--
        return preset
      }
      try {
        joined.binding.rebind(standing.key)
      } catch (error) {
        standing.joinedAgents--
        throw error
      }
      const previous = joined.standing
      joined.standing = standing
      previous.joinedAgents--
      await this.reclaimRetired(previous)
    }
    return preset
  }

  /** Bind one agent and make its scope teardown release the generation lease. */
  private bindAgent(agentCtx: Context, agentKey: ScopeKey, standing: StandingMount, retained = false): void {
    let binding: ScopeParentBinding
    try {
      binding = bindScopeParent(agentKey, standing.key)
    } catch (error) {
      if (retained) standing.joinedAgents--
      throw error
    }
    const joined: AgentStandingBinding = {
      binding,
      standing,
      released: false,
    }
    if (!retained) standing.joinedAgents++
    this.bindings.set(agentKey, joined)
    try {
      agentCtx.effect(() => async () => {
        if (joined.released) return
        joined.released = true
        this.bindings.delete(agentKey)
        joined.standing.joinedAgents--
        await this.reclaimRetired(joined.standing)
      }, 'agentPresets.standingLease()')
    } catch (error) {
      joined.released = true
      this.bindings.delete(agentKey)
      standing.joinedAgents--
  ...
  async standingKeyFor(id?: string): Promise<ScopeKey> {
    const preset = await this.resolveMountable(id)
    const standing = await this.retainCurrentStanding(preset, (current) => { current.legacyPinned = true })
    // This compatibility API predates leases. A caller can retain the bare key
    // indefinitely, so pin its generation rather than reclaiming underneath
    // it. New bounded reads should use acquireStandingKey().
    return standing.key
  }

  /**
   * Acquire a standing key for one bounded host read.
   *
   * The lease prevents a concurrently superseded generation from being
   * disposed until the read completes. Callers must release it in `finally`.
   * @param id - the preset id, or `undefined` for {@link defaultId}.
   * @returns the standing key and its idempotent asynchronous release boundary.
   * @throws when the preset is unknown or its composition is unusable.
   */
  async acquireStandingKey(id?: string): Promise<StandingKeyLease> {
    const preset = await this.resolveMountable(id)
    const standing = await this.retainCurrentStanding(preset, current => current.activeReaders++)
    let released = false
    return {
      key: standing.key,
      release: async (): Promise<void> => {
        if (released) return
        released = true
        standing.activeReaders--
        await this.reclaimRetired(standing)
      },
    }
  }

  /** Dispose a superseded generation once no agent or bounded reader uses it. */
  private async reclaimRetired(standing: StandingMount): Promise<void> {
    if (!standing.retired || standing.disposed || standing.legacyPinned) return
    if (standing.joinedAgents !== 0 || standing.activeReaders !== 0) return
    standing.disposed = true
    try {
      await standing.scope.dispose()
    } catch (error) {
      this.selfCtx.logger.warn(`agent-presets: failed to dispose superseded preset "${standing.presetId}": ${String(error)}`)
    }
  }

  /** Resolve and synchronously retain a generation before another refresh can retire it. */
  private async retainCurrentStanding(
    preset: AgentPreset,
    retain: (standing: StandingMount) => void,
  ): Promise<StandingMount> {
    while (true) {
      const standing = await this.ensureStanding(preset)
      if (standing.retired || standing.disposed) continue
      // The retain and state check share one JavaScript job: refresh cannot
      // mark the generation retired between them.
      retain(standing)
      return standing
    }
  }
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      if (this.standing.get(preset.id) === pending) {
        ...
        await this.reclaimRetired(mounted)
      }
    ...
    const created = (async (): Promise<StandingMount> => {
      ...
      try {
        ...
        this.standingByKey.set(key, standing)
        return standing
      } catch (error) {
        this.standing.delete(preset.id)
        await scope.dispose()
...
interface StandingMount {
  ...
  /** Stable id used in disposal diagnostics. */
  readonly presetId: string
  /** Live agent scopes parented to this generation. */
  joinedAgents: number
  /** Bounded host readers currently resolving registries through this key. */
  activeReaders: number
  /** A legacy bare-key caller may retain this generation indefinitely. */
  legacyPinned: boolean
  /** Whether a newer generation replaced this one. */
  retired: boolean
  /** Single-flight guard for asynchronous scope disposal. */
  disposed: boolean
}

/** A bounded host-reader reference to one standing generation. */
export interface StandingKeyLease {
  /** Registry-view scope for the duration of the read. */
  readonly key: ScopeKey
  /** Idempotently release the generation after the read completes. */
  release(): Promise<void>
}

/** One agent's mutable link to its current standing generation. */
interface AgentStandingBinding {
