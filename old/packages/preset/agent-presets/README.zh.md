...
- `ctx.agentPresets.acquireStandingKey(id?): Promise<StandingKeyLease>` 为有界宿主读取获取常驻 scope key。调用方须在 `finally` 中释放 lease；最后一个 agent 和读取方离开后，被替代的 generation 才能回收。与 `mount()` 一样拒绝损坏的 preset。
- `ctx.agentPresets.standingKeyFor(id?): Promise<ScopeKey>` 保留给持有裸 key 的兼容接口。由于无法观察释放边界，它会将该 generation 固定到整棵运行时销毁；新的有界读取应使用 `acquireStandingKey()`。
