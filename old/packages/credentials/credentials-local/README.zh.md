...
外部编辑在快照**整体替换**后按变更引用逐个发布 `credentials/updated`——磁盘上删掉的条目绝不在内存滞留。在 Chokidar 打开目标之前，提供方会对层级最深的现有祖先路径执行 realpath 解析，再拼回缺失的后缀；文件访问和诊断仍使用配置路径，从而避免 Windows 混用 8.3 别名与 libuv 的长格式事件路径。提供方自己的写入按内容识别，只发布属于该次提交的一个事件。teardown 期间，已经进入 atomic commit 的写入仍会到达磁盘和保留下来的 provider 快照；排队写入会拒绝，service 不再 live 后也不会发布事件（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-18-credentials-inflight-disposal-events.md)）。运行期文档不可读或无效时保留最后可用快照并告警；文件不存在即空存储；启动时不可读或无效则明确报错。
