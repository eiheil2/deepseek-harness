...
External edits publish `credentials/updated` per changed reference after the snapshot is replaced **wholesale** — an entry deleted on disk never lingers in memory. Before Chokidar opens the target, the provider realpaths its deepest existing ancestor and restores any missing suffix; file access and diagnostics retain the configured path, while Windows cannot mix an 8.3 alias with long-form libuv events. The provider's own writes are recognized by content and publish exactly their one commit event. During teardown, a write already inside its atomic commit still reaches disk and the retained provider snapshot, while queued writes reject and no event publishes after the service stops being live ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-18-credentials-inflight-disposal-events.md)). An unreadable or invalid document at runtime keeps the last good snapshot and warns; an absent file is an empty store; an unreadable or invalid file at boot fails loud.
