  ...
  private async write(ref: CredentialRef, value: string | undefined): Promise<void> {
    ...
    return this.enqueue(async () => {
      ...
      await withFileLock(this.spec.filename, async () => {
        ...
        // Keep this instance coherent when disposal began during the atomic
        // write, but never publish a commit after its service stopped being live.
        if (!this.isClosed()) this.notifyUpdated(ref)
      })
    })
  }
