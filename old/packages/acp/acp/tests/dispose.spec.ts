...
describe('ACP connection ownership', () => {
  ...
  it('cancels an in-flight assistant attachment read during bridge disposal', async () => {
    const script: StreamChunk[][] = []
    harness = await makeBridgeHarness({ script })
    ...
    harness.attachments!.beforeRead = async (signal) => {
      readSignal = signal
      readStarted.resolve(undefined)
      if (signal === undefined) return
      const cancellation = (): Error => signal.reason instanceof Error
        ? signal.reason
        : new Error('attachment read cancelled')
