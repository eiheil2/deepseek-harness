...
describe('E2BFileSystem atomic writes and edits', () => {
  ...
  it('preserves lone carriage returns and detects dominant CRLF beyond 4096 characters', async () => {
    const remote = new FakeRemote()
    remote.file('/workspace/file.txt', `${'x\n'.repeat(2048)}${'one\r\r\n'.repeat(3000)}`)
    const { fs } = await setup(remote)
    const target = await fs.resolve('file.txt')
    await fs.editText(target, { oldString: 'one', newString: 'ONE', replaceAll: true })
    const stored = new TextDecoder().decode(remote.nodes.get('/workspace/file.txt')?.data)
    expect(stored).toContain('ONE\r\r\n')
    expect(stored).not.toContain('ONE\r\n')
    expect(stored.startsWith('x\r\n')).toBe(true)
  })
