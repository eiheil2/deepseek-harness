...
`reasoningEfforts` 声明模型可选的思考级别。对于分档方言，每个值都是分派在协议中发送的拼写，因此 `high: high` 原样透传规范名称，而 `max: ultra` 则为使用自有词汇的网关改名。OpenAI-completions 二值方言最多提供一个正向档位，并把该值留空（`high:`），因为分派只发送启用开关，不存在可如实记录的档位拼写。键取自 pi-ai 的档位集合（`off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`）；未声明的档位不会被提供。省略该字段会保留已安装 catalog 条目的能力（手工声明的模型没有这份能力，也不推理）；`false` 声明一个不具备推理能力的模型，profile 正是以此从其网关无法服务的 catalog 模型上剥除推理；空声明会被拒绝，而不是在这两种含义之间去猜。

该声明会转换为 pi-ai 的 `Model.reasoning` + `thinkingLevelMap`，其中每个档位都被显式决定——未声明的档位一律固定为不支持，而不是留给 pi-ai 自己的默认规则：那套规则并不对称（键缺席对五个基础档位意味着「支持」，对 `xhigh`/`max` 却意味着「不支持」），也本不该要求 profile 作者了解。无值的正向档会在内部取得恒等标记，仅用于让 pi-ai 的选择器辅助函数同样看见 `xhigh`/`max`；解析只在分派忽略档位值的 OpenAI-completions 二值方言上允许该标记。`off` 缺席时不提供 Off；声明而不给值（`off:`）时采用该方言原生的禁用方式或省略参数；声明并给值（`off: none`）时，只有实际发送该值的方言才会接受它。没有任何写法能把 catalog 映射中的键恢复为「未设置」：这份声明就是对外提供的全部，因此把你要保留的 catalog 档位重述出来。

### 推理分派的 compat 开关

思考级别如何在协议中传输——单独一个 `reasoning_effort`、DeepSeek 的 `thinking: {type}` 加上档位、z.ai 的 `thinking` 对象，诸如此类——就是 pi-ai 的 `compat.thinkingFormat`，pi-ai 会从端点 URL 猜测它；私有网关的 URL 什么也说明不了，于是说 DeepSeek 方言的网关只会收到 OpenAI 方言的请求，且无从更正。因此 `compat.thinkingFormat` 与 `compat.supportsReasoningEffort` 既可配置在路由上（作为其模型的默认值），也可按模型配置（逐字段胜出），解析顺序为模型 → 路由 → 已安装 catalog 条目 → pi-ai 按 URL 得出的猜测；设置路由级开关会为路由上的每个模型遮蔽 catalog 条目的值，而且除了重述其值，没有任何写法能把某个字段交还给 catalog。

在 OpenAI-completions 模型上，配置的 `reasoningEfforts` 声明必须解析出显式的 `thinkingFormat`；URL 猜测不能证明选择器各档会生成不同请求。OpenAI 风格的分档能力要求 `supportsReasoningEffort: true`。DeepSeek、z.ai 与 Together 在该开关未显式为 true 时退化为二值，Qwen 始终是二值；这些形式只接受一个无值正向档。OpenRouter、Ant Ling 与 string-thinking 直接携带正向档拼写。解析会拒绝二值形式上的多个正向档、方言会忽略的拼写、重复的正向拼写，以及会与正向档生成相同请求的 Off 拼写。因此，该可配置协议面上的每个已接受选择器选项，都会在路由注册前生成可区分的请求形状。其他协议保留 pi-ai 自有的推理行为，并要求每个已声明正向档都提供协议拼写。

`thinkingFormat` 接受 pi-ai 可分派的各种格式，但不含两个 `chat-template` 变体：它们需要的 `chatTemplateKwargs` 本配置并不暴露。两个开关都只存在于 `openai-completions` 上——其余协议的推理形状由协议本身承载——因此在其他协议的模型上设置模型级开关会使解析失败，路由级开关会跳过其他协议的模型，而完全没有 `openai-completions` 模型的路由则会被拒绝。pi-ai compat 面的其余部分（`supportsStore`、`maxTokensField`……）保持自动检测，特意不在此处开放配置。
...
支持图片的请求会在 provider 分派前解析每个持久附件，包括嵌套在工具结果中的图片。转换通过显式栈遍历嵌套结果，因此恢复历史的深度不会消耗 JavaScript 调用栈。请求取消信号会沿水合过程传递，使遵守信号的 store 能停止进行中的读取，而且调用方取消后不会再发出 provider 请求。stream idle timer 从 provider demand 开始计时；它不是附件读取超时。
...
`baseURL` 设定该路由下每个模型的端点，因此仍支持 `https://proxy.example.com:8443` 等私有 proxy；省略它的 catalog 路由会保留每个 catalog 模型自己的端点。在 catalog 路由上点名 `api` 会把整条路由改指到该协议，这正是部署把某个提供方在 Responses 与 Chat Completions 之间迁移的方式。重定向后的 descriptor 会保留与协议无关的 catalog 字段，但丢弃旧协议的 `compat` wire 行为。其思考映射保留 `null` 能力排除信息与扩展档位可用性，同时用目标协议默认值替换异协议 wire 拼写。显式 `reasoningEfforts` 声明会在隔离之后构造完整的目标映射。
...
受支持的 profile 字段是 `apiKeyEnv`、`displayName`、`api`、`baseURL`、`models`、`modelOverrides`、`compat`、`defaultReasoningEfforts`、`defaultContextWindow`、`defaultMaxTokens`、`defaultInput`、`headers`、`reasoning`、`thinkingBudgets`、`cacheRetention`、`transport`、`timeoutMs`、`websocketConnectTimeoutMs`、`streamIdleTimeoutMs` 和 `retryPolicy`。每个 profile 的可选重试策略都会与该提供方路由一同捕获；省略时使用有界的常规默认值。三个超时值均不超过 Node 的最大定时器延迟 `2,147,483,647` 毫秒，以免宿主把更大的配置延迟钳成约一毫秒。依照 pi-ai 的 SDK 选项语义，`timeoutMs` 与 `websocketConnectTimeoutMs` 接受零；流空闲间隔必须为正，默认为五分钟，且只覆盖未完成提供方读取，不包括消费方思考时间。若已配置标头中有同名项，则以 Harness 应用归因为准。
...
pi-ai 会把 SDK `sessionId` 用作提供方可见的缓存或会话亲和元数据；其 Codex Responses 传输还以该值作为进程内 WebSocket 连接、SSE 回退决定和响应续接状态的键。适配器绝不直接转发持久的 Harness session id。同时带有 Harness session id 和显式已解析凭据的请求，会取得一个不透明的进程级 HMAC id；其输入包含 Harness session、提供方路由、模型 id、API、端点、路由本地缓存代次、有效标头和凭据身份。该 id 小于 pi-ai 的 64 字符限制；相同输入可在同一进程内复用，而该路由、其模型 descriptor、标头或凭据发生影响缓存的变化时会隔离传输状态。修改其他提供方路由不会改变本路由的代次与缓存。进程密钥与作用域原始输入均不会发送出去。没有 Harness session id，或使用提供方原生 ambient credential 的请求会省略 SDK id，因为适配器无法证明 ambient credential 的身份；这种请求放弃 SDK 会话亲和与缓存复用，包括 Codex 的持久 WebSocket、回退和续接缓存，以免跨认证上下文共享状态。

其中纳入 HMAC 的代次按路由隔离，且只保留在当前不可变 snapshot 中。某一路由或其模型 descriptor 的缓存相关配置发生变化时，只隔离该路由；移除路由会丢弃其作用域，因此重新添加时也会取得新代次，不能恢复旧的 SDK 状态；修改其他提供方路由不会阻止本路由复用 SDK 连接与响应续接状态，同时不会维护无界的路由历史映射。
...
- pi-ai 把推理 token 包含在输出 usage 中，并为公开推理明细的 provider 报告独立的推理计数。适配器将该明细保留为 `reasoningTokens`；它仍是 `outputTokens` 的子集，不会再次累加到总量。
...
只有在 loop 记录生成内容后，它才会影响后续输入。pi-ai 把推理 token 包含在输出 usage 中；它给出推理明细时，适配器会单独报告该明细，但不会再把它加进输出。
