...
export interface PiAiProviderProfile {
  ...
  transport?: Transport
  /** HTTP/provider SDK timeout in milliseconds, from zero through the Node timer maximum. */
  timeoutMs?: number
  /** WebSocket connection timeout in milliseconds, from zero through the Node timer maximum. */
...
/**
 * Keys are the offered levels; string values are wire spellings and null means
 * the resolved dialect uses no spelling for that choice. A valueless key
 * (`off:` or a binary positive level) survives validation because schemastery passes nullable data
...
const profile = z.object({
  ...
  timeoutMs: z.natural().max(MAX_TIMER_DELAY_MS),
  websocketConnectTimeoutMs: z.natural().max(MAX_TIMER_DELAY_MS),
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    for (const name of ['timeoutMs', 'websocketConnectTimeoutMs'] as const) {
      const value = source[name]
      if (value !== undefined
        && (!Number.isInteger(value) || value < 0 || value > MAX_TIMER_DELAY_MS)) {
        throw new Error(
          `llm-pi-ai: provider "${provider}" ${name} must be a non-negative integer no greater than ${MAX_TIMER_DELAY_MS}`,
        )
      }
    }
    ...
    const catalog = resolveRouteModels({
      ...
      ...source.defaultReasoningEfforts === undefined ? {} : { defaultReasoningEfforts: source.defaultReasoningEfforts },
      defaultInput,
    ...
    resolved.set(provider, {
      ...
      estimatedContextModels: catalog.estimatedContextModels,
