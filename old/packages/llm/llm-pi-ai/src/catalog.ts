 ...
 * sends for it. `off` may leave its value empty when disabling is represented
 * without an effort spelling. A binary on/off dialect also leaves its sole
 * positive level empty because enabling carries no effort value. Resolution
 * validates those empty values against the model's final compat. A level
 * absent from the dict is not offered.
 */
...
export interface PiAiCompatProfile {
  ...
  /** Whether the endpoint accepts the `developer` role; false forces pi-ai back to `system` for gateways that reject it. */
...
export interface RouteCatalogRequest {
  ...
  compat?: PiAiCompatProfile
  /** Reasoning levels inherited by models that do not declare their own map. */
...
interface ModelReasoning {
  ...
  reasoning: boolean
  /** The map dispatch reads; absent when the matching-protocol installed entry or protocol defaults apply. */
  thinkingLevelMap?: ThinkingLevelMap
}

/** Preserve capability exclusions while removing another protocol's wire spellings. */
function protocolNeutralThinkingLevels(map: ThinkingLevelMap | undefined): ThinkingLevelMap | undefined {
  if (map === undefined) return undefined
  const neutral: ThinkingLevelMap = {}
  for (const level of THINKING_LEVELS) {
    const wire = map[level]
    if (wire === null) {
      neutral[level] = null
    } else if ((level === 'xhigh' || level === 'max') && wire !== undefined) {
      // pi-ai treats absent extended levels as unsupported, so identity values
      // retain capability without carrying the original protocol's spelling.
      neutral[level] = level
    }
  }
  return Object.keys(neutral).length === 0 ? undefined : neutral
}
...
function resolveModelReasoning(
  ...
  // `undefined` means the field was omitted and may inherit the route default;
  // null is a malformed YAML empty value and must remain rejectable below.
  const efforts = entry.reasoningEfforts === undefined ? routeDefault : entry.reasoningEfforts
  if (efforts === undefined) {
    ...
    // protocol. A matching-protocol entry's map (when any) arrives through
    // the installed-model spread in the model literal.
  ...
  for (const level of THINKING_LEVELS) {
    ...
    if (wire === undefined) {
      ...
      // Binary declarations carry no wire spelling. The identity value exists
      // only so pi-ai's selector helper treats xhigh/max like the base levels;
      // validation below permits it solely where dispatch ignores the map.
      map[level] = level
    }
  }
...
/** Validate one configured OpenAI-completions reasoning offer against its resolved wire dialect. */
function validateReasoningOffer(
  provider: string,
  model: string,
  api: string,
  efforts: false | PiAiReasoningEfforts | undefined,
  compat: OpenAICompletionsCompat | undefined,
): void {
  if (efforts === undefined || efforts === false || (efforts as unknown) === null) return
  const positive = THINKING_LEVELS.flatMap((level) => {
    const wire = efforts[level]
    return level === 'off' || wire === undefined ? [] : [{ level, wire }]
  })
  if (api !== 'openai-completions') {
    const valueless = positive.find(({ wire }) => wire === null)
    if (valueless !== undefined) {
      invalid(provider, `model "${model}" reasoningEfforts.${valueless.level} needs the wire value dispatch`
        + ' should send; only a binary openai-completions dialect may leave its positive level empty')
    }
    return
  }
  const byWire = new Map<string, ModelThinkingLevel>()
  for (const { level, wire } of positive) {
    if (wire === null) continue
    const previous = byWire.get(wire)
    if (previous !== undefined) {
      invalid(provider, `model "${model}" reasoningEfforts.${previous} and reasoningEfforts.${level}`
        + ` use the same wire value ${JSON.stringify(wire)}; selectable levels must produce distinct requests`)
    }
    byWire.set(wire, level)
  }
  const format = compat?.thinkingFormat
  const supportsEffort = compat?.supportsReasoningEffort
  const offWire = efforts.off
  if (format === undefined) {
    invalid(provider, `model "${model}" declares reasoningEfforts without a resolved thinkingFormat; declare`
      + ' compat so resolution can prove its selectable levels differ on the wire')
  }

  const binaryOff = format === 'deepseek'
    || format === 'qwen'
    || format === 'zai'
    || format === 'together'
    || format === 'ant-ling'
  if (binaryOff && typeof offWire === 'string') {
    invalid(provider, `model "${model}" reasoningEfforts.off has wire value ${JSON.stringify(offWire)}, but`
      + ` thinkingFormat "${format}" represents off without that value; leave off empty`)
  }

  const binary = format === 'qwen'
    || ((format === 'deepseek' || format === 'together' || format === 'zai') && supportsEffort !== true)
  if (binary) {
    if (positive.length > 1) {
      invalid(provider, `model "${model}" thinkingFormat "${format}" only distinguishes reasoning on from off`
        + ' under its resolved compat; declare at most one level beyond "off" or enable effort transport')
    }
    const selected = positive[0]
    if (selected !== undefined && selected.wire !== null) {
      invalid(provider, `model "${model}" reasoningEfforts.${selected.level} has wire value`
        + ` ${JSON.stringify(selected.wire)}, but thinkingFormat "${format}" sends only an enable switch; leave`
        + ' the level empty')
    }
    return
  }
  ...
  const valueless = positive.find(({ wire }) => wire === null)
  if (valueless !== undefined) {
    invalid(provider, `model "${model}" reasoningEfforts.${valueless.level} needs the wire value dispatch should`
      + ' send; only a binary on/off dialect may leave its positive level empty')
  }

  if (format === 'openrouter' || format === 'ant-ling' || format === 'string-thinking') {
    if (typeof offWire === 'string' && positive.some(({ wire }) => wire === offWire)) {
      invalid(provider, `model "${model}" reasoningEfforts.off and a positive reasoning effort use the same wire`
        + ` value ${JSON.stringify(offWire)}; selectable levels must produce distinct requests`)
    }
    return
  }
  ...
  if (format === 'openai') {
    ...
    invalid(provider, `model "${model}" reasoningEfforts cannot send a positive reasoning effort with`
      + ' thinkingFormat "openai" unless supportsReasoningEffort is explicitly true')
  }
}
...
export interface RouteCatalog {
  ...
  /** Model ids whose context capacity came from the route fallback. */
  estimatedContextModels: ReadonlySet<string>
}
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const estimatedContextModels = new Set<string>()
  const models = entries.map((entry) => {
    ...
    if (entry.contextWindow === undefined && base?.contextWindow === undefined) {
      estimatedContextModels.add(entry.id)
    }
    ...
    const reasoning = resolveModelReasoning(provider, entry, base, request.defaultReasoningEfforts)
    const compat = resolveModelCompat(provider, entry, request.compat, base, api)
    const effectiveEfforts = entry.reasoningEfforts === undefined
      ? request.defaultReasoningEfforts
      : entry.reasoningEfforts
    const effectiveCompat = 'compat' in compat
      ? compat.compat
      : base?.api === api ? base.compat : undefined
    validateReasoningOffer(provider, entry.id, api, effectiveEfforts, effectiveCompat)
    const {
      compat: _foreignProtocolCompat,
      thinkingLevelMap: _foreignProtocolThinkingLevels,
      ...baseWithoutProtocolMetadata
    } = base ?? {}
    const neutralThinkingLevels = protocolNeutralThinkingLevels(_foreignProtocolThinkingLevels)
    const compatibleBase = base?.api === api
      ? base
      : {
        ...baseWithoutProtocolMetadata,
        ...(neutralThinkingLevels === undefined ? {} : { thinkingLevelMap: neutralThinkingLevels }),
      }
    return {
  ...
  return { models, configuredMaxTokens, estimatedContextModels }
}
