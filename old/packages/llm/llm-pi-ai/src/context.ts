...
/** Flatten text inside one tool result without consuming the JavaScript call stack. */
function toolResultText(blocks: readonly ContentBlock[]): string {
  const text: string[] = []
  const pending: ContentBlock[] = []
  for (let index = blocks.length - 1; index >= 0; index--) pending.push(blocks[index] as ContentBlock)
...
async function userContent(
  ...
  const frames: UserContentFrame[] = [{ blocks, index: 0, content: [] }]
  while (frames.length > 0) {
    const frame = frames[frames.length - 1] as UserContentFrame
    if (frame.index >= frame.blocks.length) {
      const nested = frame.content.every(block => block.type === 'text')
        ? frame.content.map(block => block.text).join('')
        : frame.content
      frames.pop()
      const parent = frames[frames.length - 1]
      if (parent === undefined) return nested
      if (typeof nested === 'string') {
        if (nested.length > 0) parent.content.push({ type: 'text', text: nested })
      } else {
        for (const block of nested) parent.content.push(block)
      }
      continue
    }

    const block = frame.blocks[frame.index] as ContentBlock
    frame.index += 1
    signal?.throwIfAborted()
    switch (block.type) {
      case 'text':
        if (block.text.length > 0) frame.content.push({ type: 'text', text: block.text })
        break
      case 'image': {
        const stored = signal === undefined
          ? await attachments.readImage(block.attachment)
          : await attachments.readImage(block.attachment, signal)
        signal?.throwIfAborted()
        frame.content.push({
          type: 'image',
      ...
      case 'tool-result':
        frames.push({ blocks: block.content, index: 0, content: [] })
        break
      default:
  ...
  /* v8 ignore next -- the root frame always returns through its completion branch. */
  return ''
}
 ...
 * @param onReplayDegrade - forwarded to {@link toPiAssistant} for each assistant message.
 * @param signal - request cancellation forwarded to attachment reads.
...
export function toPiContext(
  ...
  return attachments === undefined
    ...
    : toPiContextWithImages(options, attachments, onReplayDegrade, signal)
}
