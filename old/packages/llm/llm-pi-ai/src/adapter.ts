...
interface PiAiSnapshot {
  ...
  models: Models
  /** Cache-scope generations isolated to the route whose profile changed. */
  providerGenerations: ReadonlyMap<string, string>
  /** Stable cache-affecting fingerprints for comparing recreated profile maps. */
  providerFingerprints: ReadonlyMap<string, string>
}

const SESSION_SCOPE_SECRET = randomBytes(32)

/**
 * Derive the opaque SDK session-cache key for one frozen request scope.
 * Ambient credentials have no observable identity, so they deliberately get
 * no persistent SDK session. The process-local HMAC key prevents the returned
 * value from serving as an offline oracle for session or credential material.
 * @param sessionId - Harness session identity, when the caller has one.
 * @param provider - Harness provider route.
 * @param model - Frozen pi-ai model descriptor.
 * @param generation - Frozen route-local cache-scope generation.
 * @param headers - Effective model-request headers supplied by the adapter.
 * @param apiKey - Explicit credential frozen for this call.
 * @returns a fixed-length opaque session key, or undefined when reuse is unsafe.
 */
export function piSessionScopeId(
  sessionId: string | undefined,
  provider: string,
  model: Pick<Model<Api>, 'id' | 'api' | 'baseUrl' | 'headers'>,
  generation: string | number,
  headers: Readonly<Record<string, string>>,
  apiKey: string | undefined,
): string | undefined {
  if (sessionId === undefined || apiKey === undefined) return undefined
  const effectiveHeaders = new Headers(model.headers)
  for (const [name, value] of Object.entries(headers)) effectiveHeaders.set(name, value)
  const scope = JSON.stringify({
    ...
    generation,
    headers: [...effectiveHeaders.entries()].sort(([left], [right]) => left.localeCompare(right)),
    credential: apiKey,
  })
  return `dsh-${createHmac('sha256', SESSION_SCOPE_SECRET).update(scope).digest('base64url')}`
}
...
/** Deterministic JSON for the plain profile fields that can affect SDK reuse. */
...
/** Route-local cache scope identity; unrelated provider edits must not change it. */
function providerScopeFingerprint(profile: ResolvedPiAiProviderProfile): string {
  return stableJson({
    models: [...profile.piProvider.getModels()].sort((left, right) => left.id.localeCompare(right.id)).map(model => ({
      id: model.id,
      api: model.api,
      baseUrl: model.baseUrl,
    ...
    configuredMaxTokens: [...profile.configuredMaxTokens.entries()].sort(([left], [right]) => left.localeCompare(right)),
    api: profile.api,
    ...
    defaultReasoningEfforts: profile.defaultReasoningEfforts,
  })
}
...
export class PiAiAdapter extends LlmAdapter {
  ...
  private current(): PiAiSnapshot {
    ...
    const providerGenerations = new Map<string, string>()
    const providerFingerprints = new Map<string, string>()
    for (const [provider, profile] of profiles) {
      const fingerprint = providerScopeFingerprint(profile)
      const oldFingerprint = previous?.providerFingerprints.get(provider)
      const oldGeneration = previous?.providerGenerations.get(provider)
      const generation = oldFingerprint === fingerprint && oldGeneration !== undefined
        ? oldGeneration
        : randomUUID()
      providerFingerprints.set(provider, fingerprint)
      providerGenerations.set(provider, generation)
    }
    this.snapshot = { profiles, models, providerGenerations, providerFingerprints }
    return this.snapshot
  }

  /** Observe a settings update and return a rollback for a rejected transaction. */
  observeProfiles(): () => void {
    const previous = this.snapshot
    this.current()
    return () => {
    ...
    return Promise.resolve().then(() => {
      ...
      return {
        ...
        context: {
          ...
          ...profile.estimatedContextModels.has(model) ? { estimated: true } : {},
        },
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      const context = attachments === undefined
        ...
        : await toPiContext(options, attachments, onReplayDegrade, watchdog.signal)
      const headers = requestHeaders(profile.headers)
      const sessionId = piSessionScopeId(
        options.sessionId === undefined ? undefined : String(options.sessionId),
        options.provider,
        model,
        snapshot.providerGenerations.get(options.provider) ?? 0,
        headers,
        apiKey,
      )
