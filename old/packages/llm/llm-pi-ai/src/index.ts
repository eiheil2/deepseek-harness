...
export function apply(ctx: Context, config: Config): void {
  ...
  // The adapter only observes a profile set after the route registry and the
  // configurable-provider directory accept that same set. A settings write
  // can be rejected by either registry, so keep the last fully committed
  // snapshot separate from the current candidate source.
  let acceptedProfiles = profiles()
  ...
  const storedApiKey = async (provider: string | undefined): Promise<string | undefined> => {
    ...
    const profile = acceptedProfiles.get(provider)
  ...
  installSettingsSection(ctx, NS, Config, config, {
    ...
    onChange: () => {
      ...
      const previousRegistrationFacts = registeredFacts
      const previousDirectoryFacts = directoryFacts
      ...
      try {
        // Registration captures metadata by synchronously querying the adapter,
        // so expose the candidate only for this transaction. No request can
        // interleave before either commit or rollback completes.
        acceptedProfiles = profiles()
        restoreAdapter = adapter.observeProfiles()
        ensureRegistrationFacts()
        ensureDirectory()
      } catch (error) {
        acceptedProfiles = previousProfiles
        restoreAdapter?.()
        try {
          if (registration !== undefined) registration.replace([...previousProfiles.keys()])
          if (directory !== undefined) directory.replace(directoryEntries(previousProfiles))
          registeredFacts = previousRegistrationFacts
          directoryFacts = previousDirectoryFacts
        } catch (rollbackError) {
          ctx.logger.error('llm-pi-ai: failed to roll back a refused provider update')
          ctx.logger.error(rollbackError)
        }
        ctx.logger.error('llm-pi-ai: keeping the previously accepted provider set after a refused update')
