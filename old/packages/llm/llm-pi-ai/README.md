      ...
      acme-gateway:
        ...
        models:
          ...
          - id: acme-think
            ...
            maxTokens: 32768
            # Graded dialect: key = selectable level, value = wire spelling.
            reasoningEfforts:
              off:
              high: high
              max: ultra
...
`reasoningEfforts` declares a model's selectable thinking levels. On a graded dialect, each value is the spelling dispatch sends on the wire, so `high: high` passes the canonical name through while `max: ultra` renames it for a gateway with its own vocabulary. A binary OpenAI-completions dialect offers at most one positive level and leaves that value empty (`high:`), because dispatch sends only an enable switch and there is no honest effort spelling to record. Keys come from pi-ai's level set (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`); a level not declared is not offered. Omitting the field keeps the installed catalog entry's capability (a hand-declared model has none and does not reason); `false` declares a non-reasoning model, which is how a profile strips reasoning from a catalog model its gateway cannot serve; an empty declaration is refused rather than guessing between those two meanings.

The declaration translates to pi-ai's `Model.reasoning` + `thinkingLevelMap` with every level decided explicitly — undeclared levels are pinned unsupported rather than left to pi-ai's own defaulting, which is asymmetric (an absent key means "supported" for the five base levels but "unsupported" for `xhigh`/`max`) and which a profile author should not need to know. A valueless positive level receives an internal identity marker only to keep `xhigh`/`max` visible to pi-ai's selector helper; resolution permits that marker solely on a binary OpenAI-completions dialect whose dispatch ignores effort values. `off` is left out to offer no Off; declared with no value (`off:`) it uses the dialect's native disable or omission; declared with a value (`off: none`) it is accepted only by a dialect that sends that value. There is no spelling for restoring a catalog map key to "unset": the declaration is the whole offer, so restate the catalog levels you keep.

### Reasoning-dispatch compat switches

How a thinking level travels — `reasoning_effort` alone, DeepSeek's `thinking: {type}` plus effort, z.ai's `thinking` object, and so on — is pi-ai's `compat.thinkingFormat`, which pi-ai guesses from the endpoint URL; a private gateway's URL says nothing, so a DeepSeek-dialect gateway would be spoken to in the OpenAI dialect with no way to correct it. `compat.thinkingFormat` and `compat.supportsReasoningEffort` are therefore configurable on the route (its models' default) and per model (winning per field), resolving model → route → installed catalog entry → pi-ai's URL-derived guess; setting a route-level switch shadows the catalog entry's value for every model on the route, and there is no spelling for handing a field back to the catalog short of restating its value.

On an OpenAI-completions model, a configured `reasoningEfforts` declaration must resolve an explicit `thinkingFormat`; URL guessing is not evidence that selector levels differ. OpenAI-style graded effort requires `supportsReasoningEffort: true`. DeepSeek, z.ai, and Together become binary when that switch is not explicitly true, and Qwen is always binary; those forms accept one valueless positive level. OpenRouter, Ant Ling, and string-thinking carry their positive spelling directly. Resolution rejects multiple positive levels on a binary form, a spelling a form ignores, duplicate positive spellings, and an Off spelling that would produce the same request as a positive level. Thus every accepted selector choice on this configurable protocol surface has a distinguishable request shape before the route is registered. Other protocols retain pi-ai's own reasoning behavior and require a wire spelling for every positive declared level.
...
Image-capable requests resolve every durable attachment before provider dispatch, including images nested in tool results. Conversion walks nested results with an explicit stack, so restored history depth does not consume the JavaScript call stack. The request's cancellation signal is forwarded through hydration, so a cooperative store can stop an in-flight read and no provider request is issued after caller cancellation. The stream idle timer starts at provider demand; it is not an attachment-read timeout.
...
`baseURL` sets the endpoint of every model on the route, so private proxies such as `https://proxy.example.com:8443` remain supported; a catalog route that omits it keeps each catalog model's own endpoint. Naming `api` on a catalog route repoints the whole route at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions. The repointed descriptor keeps protocol-independent catalog fields but discards the old protocol's `compat` wire behavior. Its thinking map retains `null` capability exclusions and extended-level availability while replacing foreign wire spellings with target-protocol defaults. An explicit `reasoningEfforts` declaration builds a complete target map after that isolation.
...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultReasoningEfforts`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, and `retryPolicy`. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. All three timeout values stay within Node's maximum timer delay of `2,147,483,647` milliseconds, preventing larger configured delays from being clamped to about one millisecond by the host. `timeoutMs` and `websocketConnectTimeoutMs` accept zero according to pi-ai's SDK option semantics; the stream-idle interval is positive, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Harness app attribution wins a conflicting configured header name.
...
pi-ai uses the SDK `sessionId` as provider-visible cache or affinity metadata; its Codex Responses transport also keys a process-local WebSocket connection, SSE-fallback decision, and response continuation state by that value. The adapter never forwards the durable Harness session id directly. A request carrying both a Harness session id and an explicitly resolved credential receives an opaque, process-scoped HMAC id over the Harness session, provider route, model id, API, endpoint, route-local cache generation, effective headers, and credential identity. The id stays below pi-ai's 64-character limit; identical inputs reuse it within one process, while a cache-affecting change to that route, its model descriptors, headers, or credential gets a separate transport state. Changing another provider route leaves this route's generation and cache intact. The process secret and raw scope inputs are never sent. A request without a Harness session id or using provider-native ambient credentials omits the SDK id, because the adapter cannot prove the ambient credential identity; that request gives up SDK session affinity and cache reuse, including Codex's persistent WebSocket, fallback, and continuation caches, rather than risk crossing authentication contexts.

The generation included in that HMAC is route-local and retained only in the current immutable snapshot. A cache-affecting change to one route or its model descriptors isolates that route; removing a route drops its scope, so re-adding it gets a new generation and cannot revive old SDK state. A change to another provider route leaves this route's SDK connection and continuation state reusable, without an unbounded route-history map.
...
- pi-ai includes reasoning tokens in output usage and reports a separate reasoning breakdown for providers that expose one. The adapter preserves that breakdown as `reasoningTokens`; it remains a subset of `outputTokens` and is not added to totals again.
...
Generated content affects later inputs only after the loop records it. pi-ai includes reasoning tokens in output usage and, when it supplies a reasoning breakdown, the adapter reports it separately without adding it to output again.
