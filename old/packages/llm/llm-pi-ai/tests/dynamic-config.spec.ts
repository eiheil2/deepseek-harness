...
describe('request-level dynamic profiles', () => {
  ...
  it('keeps the adapter snapshot aligned when a conflicting update removes its old route', async () => {
    const dir = await home()
    await writeFile(join(dir, '.credentials.yaml'), 'PI_LIVE_KEY: live-key\n', { mode: 0o600 })
    const server = await mockServer([{ events: textEvents }])
    const ctx = await boot(dir, {
    ...
    // The candidate removes openai and claims the already-owned anthropic
    // route. The registry must reject the whole swap, while the adapter must
    // continue resolving the old openai profile from its last accepted
    // snapshot rather than the rejected candidate.
    await ctx.settings.update(NS, { providers: { anthropic: {} } })
    expect(ctx.llm.listProviders().map(provider => provider.id).sort()).toEqual(['anthropic', 'openai'])
    const result = await assemble(ctx, { provider: 'openai', model: 'gpt-4.1', messages: [] })
    expect(result.finish.kind).toBe('error')
    if (result.finish.kind === 'error') expect(result.finish.failure.code).not.toBe('NO_ADAPTER')
    expect(server.paths).toEqual(['/v1/responses'])
  })
