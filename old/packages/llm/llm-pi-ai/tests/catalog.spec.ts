...
describe('hand-declared providers', () => {
  ...
  it('sizes a model the catalog cannot describe from the route\u2019s own fallbacks', () => {
    ...
    expect(resolved.get('acme-gateway')?.estimatedContextModels.has('bare')).toBe(true)
    expect(resolved.get('acme-gateway')?.estimatedContextModels.has('sized')).toBe(false)
  })
...
describe('per-model reasoning efforts', () => {
  ...
  it('narrows a catalog model’s levels in place', () => {
    ...
    const model = modelOf({
      deepseek: { models: [{ id: catalogModel.id, reasoningEfforts: { off: null, high: null } }] },
    }, 'deepseek')
  ...
  it('inherits one provider default across undeclared models while allowing a model override', () => {
    const models = new Map(resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
        compat: { thinkingFormat: 'openai', supportsReasoningEffort: true },
        defaultReasoningEfforts: { off: null, low: 'low', high: 'high' },
        models: [
          { id: 'plain' },
          { id: 'override', reasoningEfforts: { off: null, high: 'ultra' } },
        ],
      },
    }).get('acme-gateway')?.piProvider.getModels().map(model => [model.id, model] as const) ?? [])

    expect(getSupportedThinkingLevels(models.get('plain')!)).toEqual(['off', 'low', 'high'])
    expect(getSupportedThinkingLevels(models.get('override')!)).toEqual(['off', 'high'])
  })

  it('allows a provider default to explicitly disable reasoning', () => {
    const models = resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
        defaultReasoningEfforts: false,
        models: [{ id: 'plain' }],
      },
    }).get('acme-gateway')?.piProvider.getModels() ?? []
    expect(models[0]?.reasoning).toBe(false)
    expect(getSupportedThinkingLevels(models[0]!)).toEqual(['off'])
  })
  ...
  it('rejects a declaration that offers nothing or spells a level it cannot send', () => {
    ...
    expect(declare({ high: null })).toThrow(/only a binary on\/off dialect may leave/)
...
describe('reasoning-dispatch compat switches', () => {
  ...
  it('drops installed protocol metadata when catalog models move to another protocol', () => {
    const deepseek = modelsOf({
      deepseek: {
        api: 'openai-responses',
        baseURL: 'https://gateway.test/v1',
        models: [{ id: 'deepseek-v4-flash' }],
      },
    }, 'deepseek').get('deepseek-v4-flash')
    const gemini = modelsOf({
      google: {
        api: 'openai-responses',
        baseURL: 'https://gateway.test/v1',
        models: [{ id: 'gemini-3-pro-preview' }],
      },
    }, 'google').get('gemini-3-pro-preview')

    expect(deepseek?.api).toBe('openai-responses')
    expect(deepseek?.compat).toBeUndefined()
    expect(deepseek === undefined ? [] : getSupportedThinkingLevels(deepseek)).toEqual(['off', 'high', 'max'])
    expect(gemini?.api).toBe('openai-responses')
    expect(gemini?.thinkingLevelMap).toEqual({ off: null, minimal: null, medium: null })
    expect(gemini === undefined ? [] : getSupportedThinkingLevels(gemini)).toEqual(['low', 'high'])
  })
  ...
  it('applies route switches to every openai-completions model, entries winning per field', () => {
    const models = modelsOf({
      'acme-gateway': {
        ...
        models: [
          { id: 'dialect-default', reasoningEfforts: { off: null, high: null } },
    ...
    expect(models.get('dialect-odd')?.compat).toEqual({
      thinkingFormat: 'openai', supportsReasoningEffort: false, supportsDeveloperRole: false,
    })
  })

  it('rejects reasoning offers whose resolved compat cannot distinguish their levels', () => {
    const declared = (
      thinkingFormat: LlmPiAi.PiAiThinkingFormat,
      supportsReasoningEffort: boolean,
      reasoningEfforts: NonNullable<LlmPiAi.PiAiModelProfile['reasoningEfforts']>,
    ): (() => unknown) => () => resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
        compat: { thinkingFormat, supportsReasoningEffort },
        models: [{ id: 'm', reasoningEfforts }],
      },
    })

    expect(declared('openai', false, { off: null, high: 'high' }))
      .toThrow(/cannot send a positive reasoning effort/)
    expect(declared('deepseek', false, { off: null, low: 'low', high: 'high' }))
      .toThrow(/only distinguishes reasoning on from off/)
    expect(declared('qwen', true, { off: null, low: 'low', high: 'high' }))
      .toThrow(/only distinguishes reasoning on from off/)
    expect(declared('openai', true, { off: null, low: 'same', high: 'same' }))
      .toThrow(/same wire value/)
    expect(declared('openai', true, { off: 'same', high: 'same' }))
      .toThrow(/same wire value/)
    expect(declared('deepseek', true, { off: 'none', high: 'high' }))
      .toThrow(/represents off without that value/)
    expect(declared('deepseek', false, { off: null, high: 'ignored' }))
      .toThrow(/sends only an enable switch/)
  })

  it('keeps honest binary and multi-level reasoning offers', () => {
    const models = modelsOf({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
    ...
    expect(getSupportedThinkingLevels(models.get('binary')!)).toEqual(['off', 'high'])
    expect(getSupportedThinkingLevels(models.get('graded')!)).toEqual(['off', 'low', 'high'])
  })

  it('does not extend valueless positive levels to pi-ai-owned protocols', () => {
    expect(() => resolveProfiles({
      'acme-gateway': {
        api: 'anthropic-messages',
        baseURL: 'https://acme.test',
        models: [{ id: 'm', reasoningEfforts: { off: null, high: null } }],
      },
    })).toThrow(/only a binary openai-completions dialect may leave/)
  })
...
describe('configurable-provider directory', () => {
  it('keeps the previous directory when a route collides with another adapter family', async () => {
    ...
    const refused = await assemble(ctx, {
      provider: 'deepseek-official', model: 'm', messages: [],
    })
    expect(refused.finish).toMatchObject({
      kind: 'error', failure: { code: 'NO_ADAPTER' },
    })
  })
