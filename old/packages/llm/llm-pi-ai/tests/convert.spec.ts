...
describe('toPiContext', () => {
  ...
  it('flattens nested tool-result images into the enclosing result', async () => {
    ...
    }, { readImage } as unknown as AttachmentStore, undefined, signal)

    expect(readImage).toHaveBeenCalledWith(attachment, signal)
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('preserves an explicitly reported reasoning-token breakdown', () => {
    expect(mapUsage(usage(10, 5, 0, 0, 3))).toEqual({
      inputTokens: 10,
      outputTokens: 5,
      reasoningTokens: 3,
    })
