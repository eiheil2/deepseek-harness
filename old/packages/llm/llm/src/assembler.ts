...
export class BlockAssembler {
   ...
   * truncation drops tool calls that cannot be executed safely and open block
   * types that cannot be reconstructed from deltas. Emitted blocks and replay
  ...
  private assembled(): { blocks: ContentBlock[]; replay: ReplayEnvelope | undefined } {
    const truncated = this.finish.kind === 'max-tokens'
    const all = this.order.map((index): ContentBlock | undefined => {
      const partial = this.mustGet(index)
      if (truncated && partial.block === undefined
        && partial.blockType !== 'text'
        && partial.blockType !== 'reasoning'
        && partial.blockType !== 'tool-call') return undefined
      return this.assemble(partial, index)
    })
    const kept = truncated
      ? all.map(block => block !== undefined && block.type !== 'tool-call')
      : undefined
    const blocks = all.filter((block, position): block is ContentBlock =>
      block !== undefined && (kept === undefined || kept[position] === true))
   ...
   *   tool calls that cannot be executed safely and open block types with no
   *   delta assembler; otherwise an open block assembles from its accumulated
   *   deltas (an unknown block type never closed by `block-end` throws on a
   *   non-truncated finish).
   */
  blocks(): ContentBlock[] {
  ...
  /**
   * Finish reason from the terminal `finish` chunk. A stream that ends without
   * one is a transport failure, never an implicit successful completion.
   */
  get finish(): FinishReason {
    return this._finish ?? {
      kind: 'error',
      failure: {
        message: 'LLM stream ended without a terminal finish chunk',
        code: 'STREAM_CLOSED',
      },
    }
  }
