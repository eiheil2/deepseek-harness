...
/** Define one ordinary enumerable data property without invoking `__proto__`. */
function defineCloneProperty(target: object, key: string, value: unknown): void {
...
/** Delegate non-plain values to the platform clone so the old message contract remains intact. */
function cloneStructured<T>(value: T): T {
  return structuredClone(value)
}

/** Own enumerable string properties are the properties structuredClone copies. */
...
/**
 * Clone the cyclic/shared-reference-capable message graph. Plain objects and
 * arrays are traversed iteratively so message nesting consumes heap rather
 * than the JavaScript call stack; other structured-clone values retain the
 * platform's established behavior.
 */
function detachMessageData<T>(value: T): T {
  const clones = new Map<object, object>()
  const pending: ClonePropertyTask[] = []

  const cloneValue = (candidate: unknown): unknown => {
    if (candidate === null || typeof candidate === 'undefined'
      || typeof candidate === 'boolean' || typeof candidate === 'string'
      || typeof candidate === 'number' || typeof candidate === 'bigint') return candidate
    if (typeof candidate !== 'object') return cloneStructured(candidate)
    ...
    let target: object
    let keys: string[]
    if (Array.isArray(candidate) && hasPlainArrayPrototype(candidate)) {
      keys = enumerableStringKeys(candidate)
      target = new Array(candidate.length)
    } else if (!Array.isArray(candidate) && hasPlainObjectPrototype(candidate)) {
      // structuredClone normalizes null-prototype records to Object.prototype.
      keys = enumerableStringKeys(candidate)
      target = {}
    } else {
      const clone = cloneStructured(candidate)
      clones.set(candidate, clone)
      return clone
    }

    const properties: ClonePropertyTask[] = []
    for (const key of keys) {
      const descriptor = Object.getOwnPropertyDescriptor(candidate, key)
      /* v8 ignore next -- keys came from this object's own-key snapshot. */
      if (descriptor === undefined) continue
      properties.push({ source: candidate, target, key, descriptor })
    }

    clones.set(candidate, target)
    for (let index = properties.length - 1; index >= 0; index--) {
      const property = properties[index]
      /* v8 ignore next -- the loop is bounded by the captured property count. */
      if (property !== undefined) pending.push(property)
    }
    return target
  }
  ...
  for (let task = pending.pop(); task !== undefined; task = pending.pop()) {
    const propertyValue: unknown = 'value' in task.descriptor
      ? task.descriptor.value as unknown
      : task.descriptor.get?.call(task.source) as unknown
    defineCloneProperty(task.target, task.key, cloneValue(propertyValue))
  }
  return root as T
}
...
export function freezeMessage<T extends Message>(message: T): T {
  return deepFreeze(detachMessageData(message))
}
