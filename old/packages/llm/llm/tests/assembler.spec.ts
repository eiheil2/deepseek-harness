...
describe('BlockAssembler', () => {
  ...
  it('classifies a stream that ends without finish as closed instead of successful', () => {
    const assembler = new BlockAssembler()
  ...
  it('drops an unassemblable open block at max-tokens and prunes its replay entry', () => {
    const assembler = new BlockAssembler()
    assembler.push({ type: 'block-end', index: 0, block: { type: 'text', text: 'safe prefix' } })
    assembler.push({ type: 'block-start', index: 1, blockType: 'video' } as unknown as StreamChunk)
    assembler.push({
      type: 'finish',
      reason: { kind: 'max-tokens' },
      replayState: { response: { id: 'response-1' }, blocks: ['text-meta', 'video-meta'] },
    })

    expect(assembler.blocks()).toEqual([{ type: 'text', text: 'safe prefix' }])
