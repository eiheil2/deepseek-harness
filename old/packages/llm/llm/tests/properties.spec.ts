...
describe('BlockAssembler properties', () => {
  ...
  it('finish reflects the last finish chunk, or reports a closed stream when none arrives', () => {
    fc.assert(fc.property(streamArb, (chunks) => {
      const a = feed(chunks)
      const finishes = chunks.filter(c => c.type === 'finish')
      if (finishes.length === 0) {
        expect(a.finish).toMatchObject({ kind: 'error', failure: { code: 'STREAM_CLOSED' } })
      } else {
