...
`cacheTelemetry` 只从实际携带至少一个缓存字段的用量样本累计 `reportedInputTokens`、缓存读取与缓存写入。Web 缓存命中率因此描述的是完整持久日志中「有报告」的子集，而不是全部输入 token，也不是仅针对当前所选路由。这样既不会让省略缓存遥测的提供方稀释另一提供方已报告的命中率，也保留了同一步骤从分片到最终值的替换语义。

`contextPressure` 携带可选的 `pressureTokens`（提供方报告的最新提示词规模，为未缓存输入加缓存读取与写入之和）、可选的 `projectedTokens`，以及来自最新一条 `request/context` 记录的可选 `contextWindow` 与 `contextEstimated` 来源标记。每条容量记录都会完整替换前一条容量及其来源；最新记录省略估算标记时，也会清除旧标记。提供方报告用量前两个数字都保持缺失；路由适配器未公布容量时容量也保持缺失。输出不计入其中，因此轮次流式输出期间 `pressureTokens` 保持不动，等到下一个请求报告用量时才前进。
...
这是刻意的选择。占用率百分比是面向用户的参考数字，既不是计费记录，也不是门控输入：harness 中没有任何环节依据它做决策，压缩改为直接读取 `measure()`。Web UI 用测得的压力除以最新记录的路由容量来计算占用率；选择另一模型不会改变该持久容量，直到某次请求实际采用这项选择。
...
- **pi-ai 无法区分「明确报告为零」和「缓存遥测不可用」**：其 `Usage` 约定要求缓存 bucket 必须存在，提供方 shim 又常把缺失的上游字段归一化为零。Harness 适配器会省略这些零值，而不是虚构统计覆盖，因此零命中的 pi-ai 调用不会进入 `cacheTelemetry`；一旦后续出现非零命中，显示的命中率可能高于无法获知的全输入命中率。`cacheRetention` 和模型成本元数据都不是可靠的遥测能力信号。
