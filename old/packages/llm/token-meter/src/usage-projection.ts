...
import type { CacheTelemetryProjection, ContextPressureProjection, TokenUsageProjection } from './projection.ts'
...
interface CacheTelemetryState {
  totals: CacheTelemetryProjection
  last: { turn: number; step: number; sample: CacheTelemetryProjection } | null
}

const zeroCacheTelemetry = (): CacheTelemetryProjection => ({
  reportedInputTokens: 0,
  cacheReadTokens: 0,
  cacheWriteTokens: 0,
})

const cacheSample = (usage: TokenUsage): CacheTelemetryProjection | undefined =>
...
const addCacheReplacing = (
  totals: CacheTelemetryProjection,
  previous: CacheTelemetryProjection | undefined,
  next: CacheTelemetryProjection,
): CacheTelemetryProjection => ({
  reportedInputTokens: totals.reportedInputTokens - (previous?.reportedInputTokens ?? 0) + next.reportedInputTokens,
  cacheReadTokens: totals.cacheReadTokens - (previous?.cacheReadTokens ?? 0) + next.cacheReadTokens,
  cacheWriteTokens: totals.cacheWriteTokens - (previous?.cacheWriteTokens ?? 0) + next.cacheWriteTokens,
})

/** Durable cache-token totals with same-step replacement for usage corrections. */
export const cacheTelemetryProjectionDefinition: ProjectionDefinition<'cacheTelemetry', CacheTelemetryState> = {
  key: 'cacheTelemetry',
  schema: cacheTelemetrySchema,
  init: () => ({ totals: zeroCacheTelemetry(), last: null }),
  apply: (state, event) => {
    const reported = usageOf(event)
    if (reported === undefined) return state
    const sample = cacheSample(reported.usage)
    if (sample === undefined) return state
    const { turn, step } = reported
    const previous = state.last !== null
      ...
      : undefined
    return {
      totals: addCacheReplacing(state.totals, previous, sample),
...
const pressureSchema = z.object({
  ...
  contextEstimated: z.boolean().optional(),
...
/** The usage and step identity a chunk or finalized message reports, if any. */
...
ProjectionDefinition<'contextPressure', ContextPressureState> = {
  ...
  apply: (state, event) => {
    ...
    if (event.type === 'request/context') {
      ...
      const contextEstimated = event.data.contextEstimated
      if (contextWindow !== state.contextWindow || contextEstimated !== state.contextEstimated) {
        if (contextWindow !== undefined) {
          const { contextEstimated: _previousEstimate, ...withoutEstimate } = next
          next = {
            ...withoutEstimate,
            contextWindow,
            ...contextEstimated === undefined ? {} : { contextEstimated },
          }
        } else {
          const { contextWindow: _removed, contextEstimated: _estimated, ...withoutContextWindow } = next
    ...
    if (reported !== undefined) {
      const pressureTokens = pressureFrom(reported.usage)
  ...
  view: ({ contextWindow, contextEstimated, pressureTokens, surfaceTokens, sampledSurfaceTokens }) => ({
