...
import type { CacheTelemetryProjection, ContextPressureProjection, TokenUsageProjection } from '@deepseek-ai/dsh-token-meter/client'
...
const cacheProjected = (ctx: Context, session: Session): CacheTelemetryProjection => {
  const value = ctx.sessionProjections.snapshot(session).values.cacheTelemetry
  if (value === undefined) throw new Error('cacheTelemetry projection is not registered')
  return value
}
...
describe('tokenUsage session projection', () => {
  it('does not claim cache coverage when the provider omits cache fields', async () => {
    const { ctx, session } = await harness()
    startStep(session, 1, 1)
    usageChunk(session, { inputTokens: 100, outputTokens: 4 }, 1, 1)
    expect(cacheProjected(ctx, session)).toEqual({
  ...
  it('counts only reported cache samples and replaces chunk with final sample', async () => {
    const { ctx, session } = await harness()
    startStep(session, 1, 1)
    const source = usageChunk(session, { inputTokens: 100, outputTokens: 4 }, 1, 1)
    finalUsage(session, { inputTokens: 120, outputTokens: 4, cacheReadTokens: 80 }, 1, 1, [source])
    startStep(session, 1, 2)
    usageChunk(session, { inputTokens: 50, outputTokens: 2, cacheReadTokens: 10, cacheWriteTokens: 5 }, 1, 2)
    expect(cacheProjected(ctx, session)).toEqual({
      reportedInputTokens: 170,
      cacheReadTokens: 90,
      cacheWriteTokens: 5,
    })
  })
...
function recordContext(session: Session, model: string, contextWindow?: number, contextEstimated?: boolean): void {
...
describe('contextPressure session projection', () => {
  ...
  it('clears an older estimate marker when the newest capacity omits it', async () => {
    const { ctx, session } = await harness()
    startStep(session, 1, 1)
    recordContext(session, 'estimated', 64_000, true)
    usageChunk(session, { inputTokens: 100, outputTokens: 10 }, 1, 1)
    expect(pressure(ctx, session)).toEqual({
      pressureTokens: 100, projectedTokens: 100, contextWindow: 64_000, contextEstimated: true,
    })

    recordContext(session, 'estimated', 64_000)
    expect(pressure(ctx, session)).toEqual({
      pressureTokens: 100, projectedTokens: 100, contextWindow: 64_000,
    })
  })
