...
`cacheTelemetry` carries `reportedInputTokens`, cache reads, and cache writes only for usage samples that actually contain at least one cache field. The Web cache-hit percentage therefore describes the reporting subset of the whole durable log, not every input token and not only the currently selected route. This prevents a provider that omits cache telemetry from diluting another provider's reported rate, while preserving same-step chunk-to-final replacement.

`contextPressure` carries optional `pressureTokens` — the newest provider-reported prompt size, summing uncached input plus cache reads and writes — optional `projectedTokens`, and optional `contextWindow` plus `contextEstimated` provenance from the newest `request/context` record. Each capacity record wholly replaces the previous capacity and provenance, including clearing an older estimate marker when the newest record omits it. Both token figures stay absent until a provider reports usage; capacity stays absent for a route whose adapter advertises none. Output is excluded, so `pressureTokens` holds still while a turn streams and steps forward when the next request reports its usage.
...
This is deliberate. An occupancy percentage is a user-facing reference figure, not a billing record or a gating input — nothing in the harness makes decisions from it, and compaction reads `measure()` instead. The Web UI computes occupancy by dividing measured pressure by the newest recorded route capacity; selecting another model does not change that durable capacity until a request uses the selection.
...
- **pi-ai cannot distinguish a reported zero from unavailable cache telemetry** — its `Usage` contract makes cache buckets mandatory and provider shims commonly normalize an absent wire field to zero. The Harness adapter omits those zeros rather than inventing coverage, so zero-hit pi-ai calls do not enter `cacheTelemetry`; after a later nonzero hit, the displayed rate can exceed the unknowable all-input rate. Neither `cacheRetention` nor model cost metadata is a reliable telemetry-capability signal.
