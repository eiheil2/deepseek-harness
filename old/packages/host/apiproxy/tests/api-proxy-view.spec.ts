...
describe('mux live view computation', () => {
  it('ends a slow subscriber generation instead of retaining an unbounded backlog', async () => {
    const { ctx } = await harness()
    const session = ctx.sessions.create()
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })
    const iterator = api.events.mux(
      { rpcId: RpcId('t-mux-overflow'), payload: {} },
      new AbortController().signal,
    )[Symbol.asyncIterator]()

    // Consume the subscription baseline, then stop pulling while the session
    // emits enough events to exceed the queue's fixed capacity.
    await expect(iterator.next()).resolves.toMatchObject({ done: false })
    // MAX_LIVE_FRAME_QUEUE is intentionally internal; one beyond its
    // pre-registered 4,096-frame bound must close this generation.
