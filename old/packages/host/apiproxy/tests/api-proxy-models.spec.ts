...
describe('Web session model selection', () => {
  ...
  it.each([
    { source: 'adapter default', adapterDefaults: { reasoningEffort: true }, expectedProposal: undefined, expectedResolved: 'high' },
    { source: 'explicit selection', adapterDefaults: undefined, expectedProposal: 'max', expectedResolved: 'max' },
  ] as const)('restores a logged $source reasoning effort by its recorded ownership', async ({
    adapterDefaults, expectedProposal, expectedResolved,
  }) => {
    const { ctx, agent, sessionId } = await harness()
    ...
    expect(directory.current).toEqual({
      provider: 'deepseek-official',
      model: 'deepseek-reasoner',
      ...expectedProposal === undefined ? {} : { reasoningEffort: expectedProposal },
    })
    await ctx.systemPrompt.assemble()
    const proposal = await agentEvents(ctx, agent).waterfall(
      'agent/request',
      { turn: 2, step: 0, signal: new AbortController().signal },
      () => Promise.resolve({ provider: 'seed', model: 'seed' }),
    )
    expect(proposal).toEqual({
      provider: 'deepseek-official',
      model: 'deepseek-reasoner',
      ...expectedProposal === undefined ? {} : { reasoningEffort: ReasoningEffortId(expectedProposal) },
    })
    await expect(ctx.llm.resolveCallConfig(proposal)).resolves.toMatchObject({
      reasoningEffort: ReasoningEffortId(expectedResolved),
    })
    await ctx.fiber.dispose()
  })
