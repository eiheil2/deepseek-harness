...
describe('sessions.cancel convergence', () => {
  it('does not acknowledge an ordinary session until its Agent reaches idle', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    const session = ctx.sessions.create(sid('cancel-convergence'), { meta: { cwd: '/proj' } })
    ...
    const response = api.sessions.cancel(request({ sessionId: session.id })).then((value) => {
      acknowledged = true
      return value
    })
    ...
    await expect(response).resolves.toMatchObject({ result: { ok: true, value: { accepted: true } } })
