...
describe('session/jobs change pushes', () => {
  it('pushes the owner\'s whole set on registration, stopping, and settlement', async () => {
    ...
    expect(frames.every(frame => frame.sessionId === session.id)).toBe(true)
    const statuses = frames.flatMap(frame => frame.jobs[0]?.status === undefined ? [] : [frame.jobs[0].status])
      .filter((status, index, all) => index === 0 || status !== all[index - 1])
    expect(statuses).toEqual(['running', 'stopping', 'killed'])
    // Terminal detail rides the same whole-set push; no separate signal.
    const terminal = frames.find(frame => frame.jobs[0]?.status === 'killed')
    expect(terminal?.jobs[0]?.detail).toBe('signal: SIGTERM')
    expect(terminal?.jobs[0]?.finishedAt).toBeTypeOf('number')
  })
...
describe('session/jobs registry lifecycle', () => {
  it('starts pushing when the jobs registry appears after the mux opened', async () => {
    const { ctx, session } = await harness(false)
    const proxy = api(ctx)
    const abort = new AbortController()
    const stream = proxy.events.mux({ rpcId: RpcId('t-tasks-late-registry'), payload: {} }, abort.signal)
    const frames: MuxFrame[] = []
    const drained = (async () => {
      for await (const envelope of stream) {
        frames.push(envelope.payload)
    ...
    ctx.jobs.attachController('api-proxy-test')
    ctx.jobs.start(producer('registered after mux').spec)
    session.append('turn/start', { turn: 1 })
    await drained

    expect(frames.find((frame): frame is JobFrame => frame.type === 'session/jobs')?.jobs[0]?.label)
      .toBe('registered after mux')
    void fiber
  })

  it('rebinds an open mux to a replacement jobs registry', async () => {
    const { ctx, session } = await harness(false)
    const firstFiber = await ctx.plugin(LocalJobRegistry)
    ctx.jobs.attachController('api-proxy-test-first')
    const proxy = api(ctx)
    const abort = new AbortController()
    const stream = proxy.events.mux({ rpcId: RpcId('t-tasks-replacement-registry'), payload: {} }, abort.signal)
    const frames: MuxFrame[] = []
    const drained = (async () => {
      for await (const envelope of stream) {
        frames.push(envelope.payload)
    ...
    const old = producer('old registry')
    ctx.jobs.start(old.spec)
    old.settle({ status: 'completed' })
    await Promise.resolve()
    await firstFiber.dispose()

    await ctx.plugin(LocalJobRegistry)
    ctx.jobs.attachController('api-proxy-test-replacement')
    ctx.jobs.start(producer('replacement registry').spec)
    session.append('turn/start', { turn: 1 })
    await drained

    const jobsFrames = frames.filter((frame): frame is JobFrame => frame.type === 'session/jobs')
    expect(jobsFrames.some(frame => frame.jobs[0]?.label === 'old registry')).toBe(true)
    expect(jobsFrames.some(frame => frame.jobs.length === 0)).toBe(true)
    expect(jobsFrames.some(frame => frame.jobs[0]?.label === 'replacement registry')).toBe(true)
  })

  it('publishes tasks already present when a replacement registry binds', async () => {
    const { ctx, session } = await harness(false)
    const firstFiber = await ctx.plugin(LocalJobRegistry)
    const proxy = api(ctx)
    const abort = new AbortController()
    const stream = proxy.events.mux({ rpcId: RpcId('t-tasks-prefilled-replacement'), payload: {} }, abort.signal)
    const frames: MuxFrame[] = []
    const drained = (async () => {
      for await (const envelope of stream) {
        frames.push(envelope.payload)
    ...
    await firstFiber.dispose()
    class RestoredJobRegistry extends LocalJobRegistry {
      constructor(providerCtx: Context) {
        super(providerCtx, { maxConcurrentJobsPerOwner: 10 })
      }

      override list(): JobSnapshot[] {
        return [{
          id: JobId('bash-restored'),
          kind: 'bash',
          label: 'restored before binding',
          status: 'running',
          startedAt: 1,
          reported: false,
        }]
    ...
    await ctx.plugin(RestoredJobRegistry)
    await vi.waitFor(() => {
      expect(frames.some((frame): frame is JobFrame =>
        frame.type === 'session/jobs' && frame.jobs[0]?.label === 'restored before binding')).toBe(true)
    })
    session.append('turn/start', { turn: 1 })
    await drained

    const jobsFrames = frames.filter((frame): frame is JobFrame => frame.type === 'session/jobs')
    expect(jobsFrames.some(frame => frame.jobs[0]?.label === 'restored before binding')).toBe(true)
  })
})
