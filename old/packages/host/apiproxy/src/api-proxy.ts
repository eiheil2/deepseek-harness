...
/** Maximum live frames retained for one slow WebSocket subscriber. */
export const MAX_LIVE_FRAME_QUEUE = 4_096
...
class FrameQueue<F> {
  ...
  push(item: F): void {
    if (this.done) return
    if (this.buffer.length >= MAX_LIVE_FRAME_QUEUE) {
      // A partial stream is not safe to continue. Drop the stale backlog and
      // end this generation; the browser reconnects and requests a baseline.
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Publish one registry change through the currently bound Jobs instance. */
  const publishJobs = (jobs: JobRegistry, owner: Agent | undefined): void => {
    if (owner !== undefined) {
      // The exact owner instance the fence compares against, so the push stays
      // correct while that Agent's scope is tearing down.
      broadcast({
        type: 'session/jobs',
        sessionId: owner.id,
    ...
    // An unowned task is visible to every caller, so every subscribed session's
    // set changed with it.
  ...
  const bindJobs = (jobs: JobRegistry): (() => void) =>
    jobs.onJobsChanged((owner) => { publishJobs(jobs, owner) })

  const publishJobBaselines = (jobs: JobRegistry): void => {
    for (const session of ctx.sessions.list()) {
  ...
  // ctx.inject activation is asynchronous even when Jobs already exists. Bind
  // that boot instance synchronously so a mux opened in the same turn cannot
  // miss its first change; the injected binding takes ownership once active.
  let detachJobs: (() => void) | undefined
  let detachBootJobs = (() => {
    const jobs = ctx.get('jobs')
    return jobs === undefined ? undefined : bindJobs(jobs)
  })()
  detachJobs = detachBootJobs
  ctx.inject(['jobs'], (jobsCtx) => {
    detachBootJobs?.()
    detachBootJobs = undefined
    const detach = bindJobs(jobsCtx.jobs)
    detachJobs = detach
    // A late or replacement provider may restore tasks before its listener
    // attaches. Publish its current non-empty sets once on every bind. This may
    // repeat a whole-set push already emitted through the boot listener, which
    // is safe because session/jobs carries authoritative snapshots, not deltas.
    publishJobBaselines(jobsCtx.jobs)
    jobsCtx.effect(() => () => {
      detach()
      if (detachJobs === detach) detachJobs = undefined
      // The injected listener leaves before the provider's own teardown effects,
      // so clear every client mirror here instead of relying on the old registry
      // to publish its final empty set after this binding is gone.
      for (const session of ctx.sessions.list()) {
        broadcast({ type: 'session/jobs', sessionId: session.id, jobs: [] })
      }
    }, 'api-proxy jobs carrier teardown')
  })
  ctx.effect(() => () => {
    detachJobs?.()
    detachJobs = undefined
  }, 'api-proxy jobs listener teardown')
   ...
   * @returns the scope and its bounded-reader release boundary.
   */
  async function presenterScopeFor(
    sessionId: SessionId,
    session: PresetBearingSession,
  ): Promise<PresenterScope> {
    const live = ctx.get('agents')?.get(sessionId)
    if (live !== undefined) return { scope: live, release: async () => {} }
    const presets = ctx.get('agentPresets')
    if (presets === undefined) return { scope: undefined, release: async () => {} }
    try {
      ...
      const lease = await presets.acquireStandingKey(resolveSessionPreset(session))
      ...
      return { scope: undefined, release: async () => {} }
    }
  }

  /** Presenter scope plus the read lease that protects a cold preset generation. */
  ...
  return {
    sessions: {
      ...
      async history(request) {
        ...
        try {
          ...
          const presenter = await presenterScopeFor(sessionId, sourceSession(source))
          try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        const presenter = await presenterScopeFor(sessionId, session)
        try {
    ...
    events: {
      mux(_request, signal) {
        ...
        const initialJobs = ctx.get('jobs')
        if (initialJobs !== undefined) {
          for (const session of ctx.sessions.list()) {
            const views = jobViews(initialJobs.list(ctx.agents.get(session.id)))
            if (views.length > 0) {
