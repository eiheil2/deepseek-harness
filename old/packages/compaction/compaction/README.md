...
The private per-session cache is keyed by `session.surface.replaceGeneration` and the processed surface-entry count. An unchanged generation extends the fold with unseen tail entries only; a log-only append with no new surface entry does no event reads, while a replacement generation rebuilds current membership and balances. Open calls are tracked by `callId`, so result order is irrelevant while a mismatched result, duplicate open identity, or missing event seq rejects as corrupt surface state.
