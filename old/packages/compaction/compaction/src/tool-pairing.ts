...
interface BalanceCache {
  ...
  /** Tool-call identities still awaiting results after the processed surface tail. */
  openCallIds: Set<CallId>
}

const balanceCacheBySession = new WeakMap<Session, BalanceCache>()

/** Apply one surface event to the set of calls awaiting results. */
function applyPairingEvent(openCallIds: Set<CallId>, event: SessionEvent): void {
  switch (event.type) {
    case 'assistant/message': {
      for (const block of event.data.message.content) {
        if (block.type !== 'tool-call') continue
        if (openCallIds.has(block.id)) {
          throw new Error(`tool-pairing balance: duplicate open tool-call ${block.id} at surface seq ${event.seq} (corrupt surface)`)
        }
    ...
    case 'tool/result': {
      ...
      if (!openCallIds.delete(callId)) {
        throw new Error(`tool-pairing balance: ${callId} has no matching tool-call at surface seq ${event.seq} (corrupt surface)`)
      }
      break
    }
    default:
...
function extendCache(
  ...
  const openCallIds = new Set(cache.openCallIds)
  for (const seq of tail) {
    applyPairingEvent(openCallIds, eventForSeq(events, seq))
    pendingCuts.push(openCallIds.size === 0)
  }

  tail.forEach((seq, offset) => cache.indexBySeq.set(seq, processed + offset))
  cache.cutBalanced = cache.cutBalanced.concat(pendingCuts)
  cache.openCallIds = openCallIds
  return cache
}
