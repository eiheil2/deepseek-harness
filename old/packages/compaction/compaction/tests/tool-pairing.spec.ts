...
describe('tool-pairing corrupt surfaces', () => {
  it('rejects an equal-count result whose callId does not match the assistant call', () => {
    const session = Session.create(SessionId('mismatched-call-id'))
    session.append('assistant/message', {
      turn: 1,
      step: 1,
      message: createMessage({
        role: 'assistant',
        content: [{ type: 'tool-call', id: CallId('model-call'), name: 'bash', arguments: '{}' }],
    ...
    session.append('tool/result', {
      ...
      step: 1,
      message: createToolResultMessage({
        callId: CallId('different-result'),
        content: [],
        isError: false,
      }),
    }, SURFACE)

    expect(() => toolPairingBalancedAfter(session, session.surface.nodes[1]!))
      .toThrow(/different-result.*no matching tool-call/)
  })
