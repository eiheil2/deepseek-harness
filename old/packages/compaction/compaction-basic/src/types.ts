...
export interface CompactionPolicyConfig {
  ...
  retainRatio?: number
  /** Non-negative safe-integer recent-context budget; mutually exclusive with `retainRatio`. */
  retainTokens?: number
  /** Summary provider; set together with `summarizationModel`, or inherit the conversation target. */
  summarizationProvider?: string
  /** Summary model; set together with `summarizationProvider`, or inherit the conversation target. */
  summarizationModel?: string
  /** Positive safe-integer provider generation cap for summarization. Defaults to `8192`. */
  maxTokens?: number
  /** Non-negative safe-integer extra attempts after the first compaction. Defaults to `1`. */
  compactionRetries?: number
  /** Non-negative safe-integer retries after canonical context overflow. Defaults to `1`. */
