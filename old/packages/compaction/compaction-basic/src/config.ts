...
function validatePolicy(
  ...
  if (retainTokens !== undefined) {
    assertNonNegativeSafeInteger(`${name}.retainTokens`, retainTokens)
  }
  if (retainRatio !== undefined && retainTokens !== undefined) {
    throw new Error(`${name}: retainRatio and retainTokens are mutually exclusive`)
  }
  if (maxTokens !== undefined) assertPositiveSafeInteger(`${name}.maxTokens`, maxTokens)
  if (compactionRetries !== undefined) {
    assertNonNegativeSafeInteger(`${name}.compactionRetries`, compactionRetries)
  }
  if (maxOverflowRetries !== undefined) {
    assertNonNegativeSafeInteger(`${name}.maxOverflowRetries`, maxOverflowRetries)
  }
...
function assertPositiveSafeInteger(name: string, value: unknown): asserts value is number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value <= 0) {
    throw new Error(`${name} (${String(value)}) must be a positive safe integer`)
  }
}
