...
describe('compact configuration and defaults', () => {
  ...
  it('validates common values and pressure-policy invariants', () => {
    const bad = [
      ...
      [{ retainTokens: -1 }, /non-negative safe integer/],
  ...
  it('rejects unsafe integer budgets and retry counts at every policy level', () => {
    ...
    for (const field of fields) {
      expect(() => resolveConfig({ [field]: unsafe })).toThrow(/safe integer/)
      expect(() => resolveConfig({
        modelPolicies: [{ provider: MODEL, model: MODEL, [field]: unsafe }],
      })).toThrow(/modelPolicies\[0\].*safe integer/)
    }

    expect(resolveConfig({
      retainTokens: Number.MAX_SAFE_INTEGER,
      maxTokens: Number.MAX_SAFE_INTEGER,
      compactionRetries: Number.MAX_SAFE_INTEGER,
      maxOverflowRetries: Number.MAX_SAFE_INTEGER,
    })).toMatchObject({
      retainTokens: Number.MAX_SAFE_INTEGER,
      maxTokens: Number.MAX_SAFE_INTEGER,
      compactionRetries: Number.MAX_SAFE_INTEGER,
