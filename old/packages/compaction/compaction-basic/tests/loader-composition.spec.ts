...
describe('real Loader composition', () => {
  ...
  it.each([
    ['top-level', { maxTokens: Number.MAX_SAFE_INTEGER + 1 }],
    ['model policy', {
      modelPolicies: [{
        provider: 'test-provider',
        model: 'test-model',
        retainTokens: Number.MAX_SAFE_INTEGER + 1,
      }],
    }],
  ])('rejects unsafe integers in %s config through Schemastery', async (_label, config) => {
    context = new Context()
    await context.plugin(LlmRuntime)
    await context.plugin(SessionStore)
    await context.plugin(TokenMeter)
    await expect(context.plugin(BasicCompactionEngine, config))
      .rejects.toThrow(/expected.*9007199254740991|cannot exceed 9007199254740991/i)
  })
