...
| `retainTokens` | no | Non-negative safe-integer recent surface budget kept verbatim; mutually exclusive with `retainRatio` and must be below the resolved threshold. |
...
| `maxTokens` | no (default `8192`) | Positive safe-integer provider generation cap for the summarization call; may include reasoning tokens. |
| `compactionRetries` | no (default `1`) | Non-negative safe-integer extra attempts after the first when pressure remains above threshold. |
| `maxOverflowRetries` | no (default `1`) | Non-negative safe-integer maximum retries after canonical context-window overflow; `0` disables recovery only. |
