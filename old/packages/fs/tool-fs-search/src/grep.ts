...
function formatRetainedGrep(retained: RetainedItems<GrepMatch>, spillRef?: SpillRef, diagnostic?: string): string {
  if (retained.seen === 0) return 'No matches found'
  const output = formatGrepOutput(retained, spillRef)
  ...
  const tool = defineTool({
    ...
    async execute(args, exec) {
      ...
      if (all.length === 0 && run.diagnostic !== undefined) {
        throw new SearchError(`grep search failed after producing no usable matches: ${run.diagnostic}`, 'SEARCH_FAILED')
      }
      return { matches: all, ...run.diagnostic === undefined ? {} : { warning: run.diagnostic } }
    },
