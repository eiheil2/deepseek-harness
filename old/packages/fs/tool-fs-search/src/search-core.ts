...
export interface RipgrepRun {
  ...
  workdir: string
  /** Bounded stderr diagnostic when ripgrep returned usable partial results after exit 2. */
  diagnostic?: string
}
 ...
 * success with zero results (`noMatches`), and exit 2 can return complete
 * stdout plus a bounded diagnostic for the caller to validate as partial
 * results. Invalid patterns, empty exit-2 stdout, other nonzero exits, and
 * truncated output throw a {@link SearchError}. Both launch-time failure domains are
...
export async function runRipgrep(
  ...
  if (outcome.exitCode === 2) {
    const diagnostic = stderrExcerpt(stderr.text, stderr.lossy)
    // ripgrep uses exit 2 both for invalid syntax and for traversal errors. A
    // traversal error can still leave valid records on stdout; retain those
    // only when the stream is complete and non-empty, and surface the bounded
    // diagnostic so callers cannot mistake the result for a full search.
    if (!/regex parse error|error parsing glob/i.test(diagnostic) && stdout.text.length > 0) {
      const text = completeStdout(toolName, stdout, rawOutputMaxBytes)
      return {
        stdout: text,
        noMatches: false,
        workdir,
        diagnostic: diagnostic || `ripgrep exited 2 after producing partial ${toolName} results`,
      }
    }
  }
