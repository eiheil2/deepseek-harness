...
function renderGlobPaths(paths: string[], caps: GlobToolCaps, root: string, spillRef?: SpillRef, diagnostic?: string): string {
  if (paths.length === 0) return diagnostic === undefined ? 'No files found' : `No files found\n\n(Warning: ripgrep returned partial results: ${diagnostic})`
  let output: string
  ...
  if (paths.length <= caps.maxResults) output = paths.join('\n')
  else if (!caps.sampleOverCapGlobResults) output = formatGlobPage(paths.slice(0, caps.maxResults), paths.length, spillRef, '.')
  else output = formatGlobOutput(sampleAcrossTopLevel(paths, caps.maxResults, root), paths.length, spillRef)
  return diagnostic === undefined ? output : `${output}\n\n(Warning: ripgrep returned partial results: ${diagnostic})`
}
...
export function applyGlobTool(ctx: Context, caps: GlobToolCaps): void {
  ...
  const tool = defineTool({
    ...
    async execute(args, exec) {
      ...
      if (all.length === 0 && run.diagnostic !== undefined) {
        throw new SearchError(`glob search failed after producing no usable paths: ${run.diagnostic}`, 'SEARCH_FAILED')
      }
      return { root, paths: all, ...run.diagnostic === undefined ? {} : { warning: run.diagnostic } }
    },
