...
let dir: string
let ctx: Context
let disposePlugins: Array<() => Promise<void>>
...
describe('search tools over the real subprocess service + the packaged rg', () => {
  beforeEach(async () => {
    ...
    const toolRuntime = await ctx.plugin(ToolRuntime)
    const subprocess = await ctx.plugin(LocalSubprocessRuntime)
    const search = await ctx.plugin(ToolFsSearch, { sampleOverCapGlobResults: true })
    disposePlugins.push(
      () => search.dispose(),
      () => subprocess.dispose(),
      () => toolRuntime.dispose(),
  ...
  afterEach(async () => {
    try {
      for (const dispose of disposePlugins) await dispose()
    } finally {
