...
describe('registration', () => {
  ...
  it('defaults the timeout budget to 120 seconds', async () => {
    const { ctx } = await setup()
    expect(ctx.tools.get('glob')?.timeoutMs).toBe(120_000)
    expect(ctx.tools.get('grep')?.timeoutMs).toBe(120_000)
  })
...
describe('exit semantics and failure classification', () => {
  ...
  it('retains complete glob stdout from exit 2 and reports that the result is partial', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => runResult('kept.ts\n', {
      exitCode: 2,
      stderr: { text: 'rg: missing: IO error: no such file or directory' },
    })
    const result = await call(ctx, 'glob', { pattern: '*.ts' })
    expect(result.isError).toBe(false)
    expect(text(result)).toContain('kept.ts')
    expect(text(result)).toContain('Warning: ripgrep returned partial results')
    expect(text(result)).toContain('IO error')
  })

  it('retains complete grep records from exit 2 and reports that the result is partial', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => runResult(`${matchLine('kept.ts', 3, 'needle')}\n`, {
      exitCode: 2,
      stderr: { text: 'rg: missing: IO error: no such file or directory' },
    })
    const result = await call(ctx, 'grep', { pattern: 'needle' })
    expect(result.isError).toBe(false)
    expect(text(result)).toContain('Line 3: needle')
    expect(text(result)).toContain('Warning: ripgrep returned partial results')
  })
...
describe('raw output acquisition', () => {
  ...
  it('does not accept partial exit-2 stdout when the subprocess stream was truncated', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => runResult('partial', {
      exitCode: 2,
      stdout: { lossy: true },
      stderr: { text: 'rg: missing: IO error' },
    })
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_RAW_OUTPUT_OVERFLOW' } })
  })
