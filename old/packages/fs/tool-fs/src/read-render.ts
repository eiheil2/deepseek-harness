...
function truncateLine(line: string, maxLineLength: number): string {
  ...
  const end = maxLineLength > 0
    && line.charCodeAt(maxLineLength - 1) >= 0xd800
    && line.charCodeAt(maxLineLength - 1) <= 0xdbff
    && line.charCodeAt(maxLineLength) >= 0xdc00
    && line.charCodeAt(maxLineLength) <= 0xdfff
    ? maxLineLength - 1
    : maxLineLength
  return `${line.slice(0, end)}... (line truncated to ${maxLineLength} chars)`
}
