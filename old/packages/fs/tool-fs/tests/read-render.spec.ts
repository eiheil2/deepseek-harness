...
describe('buildWindow', () => {
  ...
  describe('caps are per-request (the plugin config reaches the window)', () => {
    ...
    it('does not split a UTF-16 surrogate pair at the line cap', async () => {
      const result = await buildWindow(whole('abcd😀tail'), { offset: 1, limit: 10, maxLineLength: 5, maxBytes: READ_MAX_BYTES }, 'f')
      expect(result.lines[0]?.text).toBe('abcd... (line truncated to 5 chars)')
