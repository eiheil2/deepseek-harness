...
export async function writeFileAtomic(
  ...
  throwIfAborted(signal, 'write')
  const stagingDirName = internals.tempDirName?.(absolutePath) ?? `.dsh-${process.pid}-${randomUUID()}.tmpdir`
  const stagingDir = join(directory, stagingDirName)
  const tempName = internals.tempName?.(absolutePath) ?? 'content.tmp'
...
function detectLineEndings(raw: string): LineEndings {
  ...
  for (let index = 0; index < raw.length; index += 1) {
    ...
    if (index > 0 && raw.charCodeAt(index - 1) === 0x0d) crlfCount += 1
    else lfCount += 1
  }
 ...
 * time, for write-back. `LF` returns the content unchanged. `CRLF` expands the
 * canonical LF separators directly, preserving any lone `\r` that preceded a
 * normalized separator in the source.
