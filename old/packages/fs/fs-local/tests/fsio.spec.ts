...
describe('writeFileAtomic — temp-file safety', () => {
  ...
  it('uses bounded staging component names independent of the destination basename', async () => {
    const file = join(dir, `${'a'.repeat(220)}.txt`)
    let stagingDirName = ''
    let tempName = ''
    await writeFileAtomic(file, 'hi', undefined, undefined, {
      inspectTemp: async ({ stagingDir, tempPath }) => {
        stagingDirName = stagingDir.slice(stagingDir.lastIndexOf('\\') + 1).slice(stagingDir.lastIndexOf('/') + 1)
        tempName = tempPath.slice(tempPath.lastIndexOf('\\') + 1).slice(tempPath.lastIndexOf('/') + 1)
      },
    })
    expect(stagingDirName.length).toBeLessThanOrEqual(64)
    expect(tempName).toBe('content.tmp')
    expect(await readFile(file, 'utf8')).toBe('hi')
  })
...
describe('readForEdit + restoreLineEndings', () => {
  ...
  it('preserves a lone carriage return adjacent to CRLF during an edit', async () => {
    const file = join(dir, 'cr-crlf.txt')
    await writeFile(file, 'one\r\r\ntwo\r\n')
    const original = await readForEdit(file, file)
    expect(original).toEqual({ content: 'one\r\ntwo\n', lineEndings: 'CRLF' })
    const edited = applyLiteralEdit(original.content, 'two', 'TWO', false, file)
    expect(restoreLineEndings(edited.content, original.lineEndings)).toBe('one\r\r\nTWO\r\n')
  })

  it('detects the dominant line endings beyond the first 4096 characters', async () => {
    const file = join(dir, 'late-crlf.txt')
    const raw = `${'x\n'.repeat(2048)}${'y\r\n'.repeat(3000)}`
