...
import { verifyPackedRelease } from './tarball.ts'
...
async function main(): Promise<void> {
  ...
  const artifacts = verifyPackedRelease(family, root, directory)
  const total = String(artifacts.length)
  let published = 0
  let skipped = 0
  for (const [index, artifact] of artifacts.entries()) {
    const progress = `[${String(index + 1)}/${total}]`
    const { tarball, member: { name, version } } = artifact
