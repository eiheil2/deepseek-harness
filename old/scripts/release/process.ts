...
/** Executable and argv passed directly to `spawnSync`. */
export interface ReleaseInvocation {
  /** Native executable, never a Windows command shim. */
  readonly command: string
  /** Arguments passed to the executable. */
  readonly args: readonly string[]
  /** Preserve the constructed `cmd.exe /c` command line on Windows. */
  readonly windowsVerbatimArguments?: boolean
}

/** Package-manager shims that Windows requires `cmd.exe` to execute. */
const WINDOWS_COMMAND_SHIMS = new Set(['npm', 'pnpm'])

/**
 * Quote one argument for a non-interactive Windows command shim invocation.
 * @param value - argument passed to npm or pnpm.
 * @returns Double-quoted argument with no command expansion syntax.
 */
function quoteWindowsShimArgument(value: string): string {
  if (/[\r\n"%!]/.test(value)) {
    throw new Error(`release command argument is unsafe for a Windows command shim: ${JSON.stringify(value)}`)
  }
  return `"${value}"`
}

/**
 * Resolve npm and pnpm through `cmd.exe` on Windows; native executables remain
 * direct children on every platform. Node cannot execute `.cmd` shims without
 * a shell, and passing arbitrary argv through `shell: true` would reintroduce
 * command injection. The fixed command allowlist and quoted, expansion-free
 * arguments keep the shell surface narrow.
 * @param command - release tool name.
 * @param args - release tool arguments.
 * @param platform - host platform, injectable for the pure regression test.
 * @param comspec - Windows command interpreter path.
 * @returns Native executable and argv for `spawnSync`.
 */
export function releaseInvocation(
  ...
  const line = [command, ...args.map(quoteWindowsShimArgument)].join(' ')
  return {
    command: comspec,
    args: ['/d', '/s', '/c', `"${line}"`],
