...
describe('release families', () => {
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    ...
    const web = member('apps/web', '@deepseek-ai/dsh-web-frontend')
    expect(() => { dsh.validatePayload(web, ['package/package.json']) }).toThrow(/without dist\/index.html/)
    expect(() => { dsh.validatePayload(web, ['package/dist/index.html']) }).toThrow(/without a built dist asset/)
    expect(() => {
      dsh.validatePayload(web, ['package/dist/index.html', 'package/dist/assets/index.js'])
    }).not.toThrow()
