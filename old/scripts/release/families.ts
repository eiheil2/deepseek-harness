...
/** Web application artifact whose package is only useful with its built shell. */
...
class DshFamily extends ReleaseFamily {
  ...
  validatePayload(member: ReleaseMember, files: readonly string[]): void {
    ...
    if (member.name !== WEB_FRONTEND_PACKAGE) return
    if (!files.includes('package/dist/index.html')) {
      throw new Error(`${member.name} packed without dist/index.html`)
    }
    if (!files.some(file => file.startsWith('package/dist/assets/') && !file.endsWith('.map'))) {
      throw new Error(`${member.name} packed without a built dist asset`)
    }
  }
