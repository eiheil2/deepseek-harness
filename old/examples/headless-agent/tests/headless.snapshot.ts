...
const toolSchemaBudgetOverlayPath = fileURLToPath(new URL('./fixtures/tool-schema-budget.cordis.yml', import.meta.url))
const toolSchemaBudgetFailureExpected = join(snapshotsDir, 'tool-schema-budget', 'stderr.expected.txt')
...
describe('headless stream-json snapshots', () => {
  ...
  it('rejects an oversized final tool-schema set before model dispatch', async () => {
    const result = await runLoaderSmoke({
      label: 'product headless profile tool-schema budget snapshot',
      tempDirPrefix: 'headless-snapshot-tool-schema-budget-',
      binScript: dshBinScript,
      configPath: toolSchemaBudgetOverlayPath,
      binArgs: ['--profile', 'headless', '--patch', toolSchemaBudgetOverlayPath, 'This task must not reach the model.'],
      tsconfigPath,
    ...
    expect(result.stdout).toBe('\n')
    await expect(result.stderr).toMatchFileSnapshot(toolSchemaBudgetFailureExpected)
  }, 120_000)
