...
describe('toStreamChunks', () => {
  ...
  it('maps text events to text blocks', async () => {
    ...
    expect(chunks).toEqual([
      ...
      { type: 'usage', usage: { inputTokens: 3, outputTokens: 2, cacheReadTokens: 0, cacheWriteTokens: 0, cacheTelemetry: 'available' } },
      {
        type: 'finish',
        reason: { kind: 'stop' },
        replayState: {
  ...
  it('maps tool-call events, re-stringifying parsed arguments', async () => {
    ...
    expect(chunks).toEqual([
      ...
      { type: 'usage', usage: { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, cacheTelemetry: 'available' } },
      {
        type: 'finish',
  ...
  it('maps error events to error finish chunks (in-stream error style)', async () => {
    ...
    expect(chunks).toEqual([
      { type: 'usage', usage: { inputTokens: 1, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, cacheTelemetry: 'available' } },
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps cache fields and explicit availability, including zero values', () => {
    ...
    expect(mapUsage(usage(10, 5))).toEqual({
      inputTokens: 10, outputTokens: 5, cacheReadTokens: 0, cacheWriteTokens: 0, cacheTelemetry: 'available',
    })
  })
})
